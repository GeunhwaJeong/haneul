...
if [ -z "$BIN" ]; then
  ...
  echo "Build it first, e.g.: scripts/simtest/cargo-simtest simtest --profile ci -p haneul-e2e-tests"
  exit 1
fi
...
run() {
  ...
  # output comes from test_net_determinism's node/sim logs (haneul targets); the other tests
