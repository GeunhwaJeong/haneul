...
TX_FLAGS="--gas-prices="0.03uhaneul" --gas 5000000 -y -b block --node http://localhost:26657 --output json --keyring-backend test --chain-id local-1"
export JUNOD_COMMAND_ARGS="$TX_FLAGS --from juno1"

TX_FLAGS2="--gas-prices="0.03uhaneul" --gas 5000000 -y -b block --node $JUNOD_NODE2 --output json --keyring-backend test --chain-id local-2"
...
# send the Tx
junod tx wasm execute $CW20_CONTRACT $MSG --gas-prices="0.03uhaneul" --gas 5000000 -y -b block --node http://localhost:26657 --output json --keyring-backend test --chain-id local-1 --from juno1
