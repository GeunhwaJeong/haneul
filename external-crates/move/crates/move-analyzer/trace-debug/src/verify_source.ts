 ...
 * `haneul client verify-source <pkg> --verify-only <ON_CHAIN_ID>` (available since
 * haneul v1.77.0), which compares the modules already compiled under the package's
...
/**
 * Minimum Haneul binary version supporting the verification check (the CLI's
 * `--verify-only` mode was introduced in haneul v1.77.0).
 */
const MIN_HANEUL_VERSION = '1.77.0';

/**
 * How long to wait for the `haneul --version` pre-flight check.
 */
 ...
 * replay tool records in its cache summary and the environment aliases the Haneul
...
/**
 * Path to the Haneul binary used for source verification. Reads the Move IDE
 * extension's `move.haneul.path` setting (settings are a shared store, so the
 * value is readable here whether or not that extension is installed or active)
 * and defaults to a binary named `haneul` on the system path. Since verficaction
 * only produces warnings, the consequence of this extension being installed
 * independently of the Move IDE extension (and thus having no `move.haneul.path`
 * setting) is not fatal and simply means that verification will not be run.
 */
function resolveHaneulPath(): string {
    const haneulBin = process.platform === 'win32' ? 'haneul.exe' : 'haneul';
    const haneulPath = vscode.workspace.getConfiguration('move').get<string | null>('haneul.path') ?? haneulBin;
...
async function runVerification(
    ...
    const haneulPath = resolveHaneulPath();
    ...
    const haneulVersion = semanticVersion(await version(haneulPath, ['--version']));
    if (haneulVersion !== null && semver.lt(haneulVersion, MIN_HANEUL_VERSION)) {
        vscode.window.showWarningMessage(
            cannotVerifyPrefix(pkgRoot)
            + `the Haneul binary is too old (v${haneulVersion.version}) - `
            + `v${MIN_HANEUL_VERSION} or later is required.`
        );
        return;
    }
    ...
    return new Promise(resolve => {
        ...
        const child = cp.execFile(
            ...
            (error, stdout, stderr) => {
                ...
                console.log(
                    `Source verification ('${haneulPath} ${args.join(' ')}') finished with `
...
function verificationFailureMessage(
    ...
    if (error.code === 'ENOENT') {
        return cannotVerify
            + `unable to locate the Haneul binary (set 'move.haneul.path' or add 'haneul' `
            + `to the system path).`;
    }
    if (error.killed) {
