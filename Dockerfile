...
# then log output of file /code/bin/haneuld
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/bin/haneuld \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/bin/haneuld | grep "statically linked")

# --------------------------------------------------------

FROM alpine:3.21

COPY --from=go-builder /code/bin/haneuld /usr/bin/haneuld
