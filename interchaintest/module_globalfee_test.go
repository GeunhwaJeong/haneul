...
import (
	...
	helpers "github.com/GeunhwaJeong/haneul/tests/interchaintest/helpers"
...
func TestJunoGlobalFee(t *testing.T) {
	...
	cfg := junoConfig
	cfg.GasPrices = "0.003uhaneul" // this is used in the faucet cmd, must match initial globalfee
	...
	// success: validate the new value is in effect (200k gas * 0.005 = 200uhaneul)
...
func bankSendWithFees(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, from ibc.Wallet, toAddr, coins, feeCoin string, gasAmt int64) string {
	cmd := []string{
		"haneuld", "tx", "bank", "send", from.KeyName(), toAddr, coins,
		"--home", chain.HomeDir(),
