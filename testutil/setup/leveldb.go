...
import (
	...
	junoapp "github.com/GeunhwaJeong/haneul/app"
...
func SetupTestingAppWithLevelDB(isCheckTx bool) (app *junoapp.App, cleanupFn func()) { // nolint:revive
	dir, err := os.MkdirTemp(os.TempDir(), "haneul_leveldb_testing")
	if err != nil {
		panic(err)
	}
	leveldb, err := db.NewGoLevelDB("haneul_leveldb_testing", dir, nil)
	if err != nil {
		panic(err)
	}

	app = junoapp.New(log.NewNopLogger(), leveldb, nil, true, dir, sims.EmptyAppOptions{}, []wasmkeeper.Option{}, baseapp.SetChainID("haneul-1"))
	if !isCheckTx {
