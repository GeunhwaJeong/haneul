...
import (
	...
	"github.com/GeunhwaJeong/haneul/testutil/common"
	"github.com/GeunhwaJeong/haneul/testutil/setup"
)
...
func (suite *KeeperTestHelper) Setup() {
	...
	dir, err := os.MkdirTemp("", "haneuld-test-home")
	if err != nil {
	...
	if common.IsDebugLogEnabled() {
		suite.App = setup.Setup(false, dir, "haneul-1", suite.T())
	} else {
		suite.App = setup.Setup(false, dir, "haneul-1")
	}

	suite.Ctx = suite.App.NewContextLegacy(false, tmtypes.Header{Height: 1, ChainID: "haneul-1", Time: defaultTestStartTime})
...
func (suite *KeeperTestHelper) SetupTestForInitGenesis() {
	dir, _ := os.MkdirTemp("", "haneuld-test-home")
	// Setting to True, leads to init genesis not running
	suite.App = setup.Setup(true, dir, "haneul-1")
