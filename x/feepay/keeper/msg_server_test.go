...
import (
	...
	"github.com/GeunhwaJeong/haneul/x/feepay/types"
)
...
func (s *KeeperTestSuite) TestFundFeePayContract() {
	...
	_, _, sender := testdata.KeyTestPubAddr()
	_, _, admin := testdata.KeyTestPubAddr()
	s.FundAcc(sender, sdk.NewCoins(sdk.NewCoin("stake", sdkmath.NewInt(1_000_000)), sdk.NewCoin("uhaneul", sdkmath.NewInt(100_000_000))))
	s.FundAcc(admin, sdk.NewCoins(sdk.NewCoin("stake", sdkmath.NewInt(1_000_000))))
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			contractAddress: "Invalid",
			senderAddress:   sender.String(),
			amount:          sdk.NewCoins(sdk.NewCoin("uhaneul", sdkmath.NewInt(1_000_000))),
			shouldErr:       true,
		},
		{
			desc:            "Fail - Invalid Sender Address",
		...
		},
		{
			...
			contractAddress: contract,
			senderAddress:   sender.String(),
			amount:          sdk.NewCoins(sdk.NewCoin("uhaneul", sdkmath.NewInt(100_000_000_000))),
			shouldErr:       true,
		},
		{
