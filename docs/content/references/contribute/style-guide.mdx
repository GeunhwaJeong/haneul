...
goal:
  description: >-
    Contributor understands the style guide requirements for the Haneul
    documentation
  requires:
    - has_frontmatter:
        - title
...
questions:
  - What is Style Guide in Haneul?
  - Where can I find the style guide reference?
  - How does style guide work on Haneul?
answer: All contributions to the Haneul documentation must adhere to the style guide.
---
...
- **Use simple words and concise sentences.**
  Prefer plain, direct language over complex or academic phrasing. Short sentences improve readability and are easier to localize. Eliminate filler words and reduce sentence length wherever possible.

- **Do not start introductions with "this page" or "this guide".**
  Opening lines like "This page explains..." or "This guide walks you through..." are poor for answer engine optimization (AEO) and generative engine optimization (GEO). Lead with the topic itself, not a meta-reference to the document. For example, write "Every agent writes durable memory objects to two systems..." instead of "This page explains how agents write durable memory objects."
...
Only capitalize terms that are official product names. If a term is not an established product name, use lowercase. For example, write "security delete API" unless "Security Delete API" is the official branded name of that feature.
...
Always capitalize proper nouns and product names. See [words to always capitalize](#terminology-and-vocabulary) for the exhaustive list of capitalized terms. 
...
Headings and titles must be descriptive enough to distinguish pages in search results, but concise. Avoid one-word titles (for example, "Installing") that give no context. Also avoid overly long or wordy headings. Prefer short, direct phrases: "Renew memories" rather than "How to renew your stored memories." If a heading can say the same thing in fewer words, shorten it.
...
### Numbered subheading consistency

If you number steps in prose (for example, "there are three stages"), the corresponding subheadings must also be numbered ("1. Initialize," "2. Configure," "3. Deploy"). Do not describe a numbered sequence in text and then use unnumbered subheadings, or vice versa.
...
Use a prerequisites section to list what the reader must have installed, configured, or completed before starting a procedure. Do not use prose headings like "Before you begin" or "Before you start." Use the prerequisite tab component instead, which provides a consistent, scannable format across all pages.
...
Always use full, relative links when linking to topics on [docs.haneulfoundation.org].
...
> Before you install Haneul, make sure to install the [prerequisites](/getting-started/onboarding/haneul-install).
...
Do not manually add footer sections such as "Related topics," "Next steps," or "Related links" to the bottom of a page. These sections are autogenerated by the site build. Manually written footer sections create duplicates and go stale when pages are renamed or moved.
