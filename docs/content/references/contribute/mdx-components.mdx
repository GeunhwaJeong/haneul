...
goal:
  description: >-
    Contributor can use the custom MDX components available in Haneul docs, such as
    tabs, admonitions, and ImportContent
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is MDX Components in Haneul?
  - Where can I find the mdx components reference?
  - How does mdx components work on Haneul?
answer: Extensions to the Haneul docs rendering engine.
---
...
```jsx
<UnsafeLink href="/getting-started">Link title</UnsafeLink>
```
...
The docs use a few custom scripts to properly render and style the `<Tabs>` component. These scripts let you use `<Tabs>` with [`{@include:}` snippets](#snippet-mode) without breaking the page's formatting.
...
Reference files through their path from the repository root, without a leading `haneul/`. For example, `source="examples/move/hero/sources/example.move"`.
...
```jsx
<ImportContent source="I2/fixed_supply/sources/silver.move" org="HaneulLabs" repo="haneul-move-bootcamp" branch="solution" mode="code" />
```
