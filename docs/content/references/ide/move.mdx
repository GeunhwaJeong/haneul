...
description: >-
  The Move extension provides language support features like code navigation,
  completion, and diagnostics for Move source code.
keywords:
  - move analyzer
  - analyze move
  - syntax highlighting
...
goal:
  description: >-
    Reader can set up and use move analyzer vs code extension for a better Move
...
questions:
  - What is Move Analyzer VS Code Extension in Haneul?
  - Where can I find the move analyzer vs code extension reference?
  - How does move analyzer vs code extension work on Haneul?
answer: >-
  The Move Analyzer extension for Visual Studio Code provides language support
...
### Install the Haneul binary

Several features of the Move extension and the extensions included with it use the `haneul` binary: the build, test, and trace commands, as well as source verification in the [Move Trace Debugger](/references/ide/debugger). See [Install Haneul](/getting-started/onboarding/haneul-install) for installation instructions.

The extensions search for the `haneul` binary on the system path. To use a binary in a different location, set the `move.haneul.path` setting to point to it.
...
To execute these commands you must have the `haneul` binary [installed](#install-the-haneul-binary); to generate a trace, the binary must also be trace-generation capable (see [Debugger](/references/ide/debugger#install)).
