...
description: >-
  Reference for all Haneul Foundation-managed checkpoint stores, snapshot buckets, and
  archival endpoints available for Haneul Mainnet and Testnet, including retention
...
keywords:
  ...
  - formal snapshot
  - archival fallback
  - haneullabs-mainnet-checkpoints
  - haneullabs-testnet-checkpoints
  - data sources
goal:
  description: >-
    Operator can identify the correct Haneullabs-managed data store for their use
...
questions:
  - What checkpoint stores are available for Haneul?
  - Where can I download Haneul snapshots?
  - What is the difference between public HTTPS endpoints and Requester Pays buckets?
  - What credentials do I need for Haneullabs-managed data stores?
  - Which data store should I use for indexer backfill?
answer: >-
  Haneul Labs manages two categories of data stores for Haneul operators:
...
Haneul Foundation manages several categories of data stores for Haneul operators and builders. Each store serves a specific purpose, has its own retention policy, and might require credentials for access.
...
| --- | --- | --- |
| Mainnet | `gs://haneullabs-mainnet-checkpoints-use4` | `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` |
| Testnet | `gs://haneullabs-testnet-checkpoints-use4` | `--remote-store-gcs haneullabs-testnet-checkpoints-use4` |
...
| --- | --- | --- |
| Mainnet | `https://s3.us-west-2.amazonaws.com/haneullabs-mainnet-checkpoints` | Use as `ingestion-url` in `fullnode.yaml` |
...
The Haneul S3 client defaults `request_payer` to `false`. You must explicitly enable it in the archival fallback configuration:
...
| **Network** | **URL** |
| --- | --- |
| Mainnet | `https://checkpoints.mainnet.haneulfoundation.org` |
| Testnet | `https://checkpoints.testnet.haneulfoundation.org` |
...
These endpoints are public-good services for testing only. Do not use them for production backfill, recovery, or steady-state indexer operation. State sync explicitly rejects `checkpoints.mainnet.haneulfoundation.org` as an archive source. For full-retention access, use the [GCS](#checkpoint-gcs) or [S3](#checkpoint-s3) buckets.
...
| **Network** | **URL** |
| --- | --- |
| Mainnet | `https://formal-snapshot.mainnet.haneulfoundation.org` |
| Testnet | `https://formal-snapshot.testnet.haneulfoundation.org` |
...
- [Full node formal snapshot restore](/operators/snapshots) with `haneul-tool download-formal-snapshot`.

### Formal snapshot buckets (Requester Pays) {#formal-buckets}

| **Network** | **S3** | **GCS** |
| --- | --- | --- |
| Mainnet | `s3://haneullabs-mainnet-formal/` | `gs://haneullabs-mainnet-formal/` |
| Testnet | `s3://haneullabs-testnet-formal/` | `gs://haneullabs-testnet-formal/` |
...
Haneul Labs hosts free formal snapshots on Cloudflare R2 (currently in North America only). No credentials are required. Access through `haneul-tool download-formal-snapshot --no-sign-request`.
