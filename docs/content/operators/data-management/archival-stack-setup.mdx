...
questions:
  - How do I set up the Haneul Archival Store and Service?
  - What are the components of the Haneul archival stack?
...
- A checkpoint bucket for your network, for history:
  - **Mainnet:** `--remote-store-gcs haneullabs-mainnet-checkpoints-use4` (Requester Pays).
  - **Testnet:** `--remote-store-gcs haneullabs-testnet-checkpoints-use4` (Requester Pays).
- A [Haneul full node](/operators/full-node/haneul-full-node) with gRPC enabled, for the tip.
...
The public HTTPS endpoints (`https://checkpoints.mainnet.haneulfoundation.org`, `https://checkpoints.testnet.haneulfoundation.org`) keep only the latest 30 days. Use the GCS buckets above for backfill.
...
**Do not use Bigtable replication for this deployment.** Run a separate Bigtable instance, `haneul-kvstore-alt` indexer, and `haneul-kv-rpc` service in each region.

### Create tables

Create these 14 tables, each with a single column family `haneul` and `maxversions=1`:
...
`haneul-kvstore-alt` reads checkpoints and writes to Bigtable through 14 pipelines.
...
`haneul-kv-rpc` is a gRPC server that reads from Bigtable and exposes the [`LedgerService`](/references/fullnode-protocol) API.

### Run `haneul-kv-rpc`

```sh
haneul-kv-rpc --config /etc/haneul-kv-rpc/config.toml
```
...
Run `haneul-kv-rpc --config-schema` to print the full JSON Schema with per-field documentation.
...
Same [`LedgerService`](/references/fullnode-protocol) API as a Haneul full node, so existing gRPC clients work by changing the endpoint URL. Server reflection is enabled.
