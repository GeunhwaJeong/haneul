...
- Access to a checkpoint source (see [Available Data Stores](/operators/data-management/available-data-stores) for the full list):
   - **Full-retention GCS buckets (recommended for backfill):** `gs://haneullabs-mainnet-checkpoints-use4` or `gs://haneullabs-testnet-checkpoints-use4` ([Requester Pays](/operators/data-management/available-data-stores#checkpoint-gcs)).
...
For Testnet, use `--remote-store-gcs haneullabs-testnet-checkpoints-use4`. Both buckets are [GCS Requester Pays](/operators/data-management/available-data-stores#checkpoint-gcs). Do not use the public HTTPS endpoints (`checkpoints.mainnet.haneulfoundation.org` / `checkpoints.testnet.haneulfoundation.org`) for full-retention backfill because they retain only the most recent 30 days.
