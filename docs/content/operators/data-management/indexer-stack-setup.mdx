...
For checkpoint ingestion, the public `https://checkpoints.mainnet.haneul.io` and `https://checkpoints.testnet.haneul.io` stores are for testing only. Do not use them for production backfill, recovery, fallback, or steady state. See [Checkpoint Data Sources](/develop/accessing-data/custom-indexer/indexer-data-integration#checkpoint-data-sources) for the source matrix.
...
Mainnet and Testnet restores can use `https://checkpoints.mainnet.haneul.io` or `https://checkpoints.testnet.haneul.io` for `--remote-store-url`, respectively.
...
| `--http`, `--gcs`, `--azure`, `--s3`, or `--local` | Formal snapshot source. Specify exactly one. See [managed snapshots](https://docs.haneul.io/operators/snapshots#haneullabs-labs-managed-snapshots). |
...
For Testnet, use `https://formal-snapshot.testnet.haneul.io`, `https://checkpoints.testnet.haneul.io`, and a production-grade Testnet fullnode endpoint from an RPC provider. Use the `haneullabs-testnet-checkpoints-use4` bucket when GCS catch-up is necessary. Keep every source on the same network and use a separate RocksDB path per network.
