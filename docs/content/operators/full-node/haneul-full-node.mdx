...
description: >-
  Operate a Haneul full node to validate blockchain activities, like transactions,
...
goal:
  description: >-
    Node operator understands operate a Haneul full node to validate blockchain
...
questions:
  - How do I set up a Haneul full node?
  - What are the hardware requirements for a Haneul full node?
  - How do I configure a Haneul full node for production?
answer: >-
  Operate a Haneul full node to validate blockchain activities, like transactions,
  checkpoints, and epoch changes.
---
...
5. Verify connectivity and query the historical range required by your clients. `end_checkpoint` is exclusive. Paginate past any `ITEM_LIMIT` responses and confirm that the stream terminates with `CHECKPOINT_BOUND`. Compare the latest checkpoint returned by `GetCheckpoint` with the node's `last_executed_checkpoint` metric before cutover:
    ```sh
    $ grpcurl -plaintext <YOUR_NODE>:9000 haneul.rpc.v2.LedgerService/GetCheckpoint
    $ grpcurl -plaintext -d '{
        "start_checkpoint": "<FIRST_REQUIRED_CHECKPOINT>",
        "end_checkpoint": "<LAST_REQUIRED_CHECKPOINT_PLUS_ONE>"
      }' <YOUR_NODE>:9000 haneul.rpc.v2.LedgerService/ListTransactions
    ```
...
A Haneul full node uses the following ports by default:
...
1. **Check sync progress.** Compare your node's latest checkpoint against a trusted endpoint. A growing gap indicates the node is still catching up.
    ...
    $ grpcurl fullnode.mainnet.haneulfoundation.org:443 haneul.rpc.v2.LedgerService/GetCheckpoint \
        | grep sequenceNumber
    ```
...
1. Download the `docker-compose.yaml` file:
    ...
    ```sh
    $ curl -fLO https://raw.githubusercontent.com/GeunhwaJeong/haneul/branch-name/docker/fullnode/docker-compose.yaml
    ```
...
1. For Testnet or Mainnet: Edit the `fullnode.yaml` file to include peer nodes for state synchronization. Append the following to the end of the current configuration:
    ...
    p2p-config:
      seed-peers:
        - address: /dns/mel-00.mainnet.haneulfoundation.org/udp/8084
          peer-id: d32b55bdf1737ec415df8c88b3bf91e194b59ee3127e3f38ea46fd88ba2e7849
        - address: /dns/ewr-00.mainnet.haneulfoundation.org/udp/8084
          peer-id: c7bf6cb93ca8fdda655c47ebb85ace28e6931464564332bf63e27e90199c50ee
        - address: /dns/ewr-01.mainnet.haneulfoundation.org/udp/8084
          peer-id: 3227f8a05f0faa1a197c075d31135a366a1c6f3d4872cb8af66c14dea3e0eb66
        - address: /dns/lhr-00.mainnet.haneulfoundation.org/udp/8084
    ...
    p2p-config:
      seed-peers:
        - address: /dns/yto-tnt-ssfn-01.testnet.haneulfoundation.org/udp/8084
          peer-id: 2ed53564d5581ded9b6773970ac2f1c84d39f9edf01308ff5a1ffe09b1add7b3
        - address: /dns/yto-tnt-ssfn-00.testnet.haneulfoundation.org/udp/8084
          peer-id: 6563732e5ab33b4ae09c73a98fd37499b71b8f03c27b5cc51acc26934974aff2
        - address: /dns/nrt-tnt-ssfn-00.testnet.haneulfoundation.org/udp/8084
...
1. Stop the node.
2. Download and restore the latest snapshot. The [`haneul-tool download-formal-snapshot` command](/operators/snapshots#formal-snapshots) requires `--genesis`, `--path`, and `--network` flags in addition to `--latest`.
3. Verify you placed the snapshot at the correct `db_path` (see your `fullnode.yaml` configuration).
4. Restart the node.

For the complete procedure, see [Haneul Snapshots](/operators/snapshots).

### Version upgrades

Haneul releases new versions regularly. To upgrade your node:

1. Check the target version in the [release notes](https://github.com/GeunhwaJeong/haneul/releases) and verify compatibility with your network.
2. Stop the node.
3. Replace the `haneul-node` binary with the new version. If you installed with `haneulup`, run `haneulup update`.
...
| Symptom | Cause | Fix |
|---|---|---|
| Node cannot start after upgrade | Configuration format changed between versions | Check release notes for `fullnode.yaml` changes. Compare your config with the [template](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-config/data/fullnode-template.yaml). |
...
| `genesis.blob` chain mismatch | Wrong genesis file for target network | Download the correct `genesis.blob` from [haneul-genesis](https://github.com/GeunhwaJeong/haneul-genesis). See [Genesis](/operators/genesis#network-identity). |
