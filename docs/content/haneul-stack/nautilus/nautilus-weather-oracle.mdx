...
description: >-
  Build a verified weather oracle on Haneul using Nautilus, where a Trusted
  Execution Environment fetches weather data, signs it cryptographically, and
  the Move contract mints an NFT only after verifying the signature.
keywords:
  - example
  - nautilus
  - tee
...
last_verified: 2026-04-29T00:00:00.000Z
...
concepts:
  ...
  - enclave_attestation
goal:
  description: Reader can build a verified weather oracle on Haneul using Nautilus
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I build a weather oracle with Nautilus on Haneul?
  - What does the Nautilus weather oracle example demonstrate?
  - How do I run offchain computation and verify it onchain?
answer: >-
  Build a verified weather oracle on Haneul using Nautilus, where a Trusted
  Execution Environment fetches weather data, signs it cryptographically, and
  the Move contract mints an NFT only after verifying.
---
...
- [x] Work through [Using Nautilus](/haneul-stack/nautilus/using-nautilus) and confirm you can deploy an enclave to an AWS Nitro instance.

- [x] [Install the Haneul CLI](/getting-started/onboarding/haneul-install) and [fund an address with Testnet HANEUL](/getting-started/onboarding/get-coins).

- [x] An API key for the data source you bring onchain. The reference app uses [weatherapi.com](https://www.weatherapi.com/).
...
A complete implementation lives in [`haneul-move-bootcamp/K4`](https://github.com/GeunhwaJeong/haneul-move-bootcamp/tree/solution/K4). Only 2 files change for any Nautilus application:

```
K4/example/
├── move/
│   └── weather.move           # Signature verification + minting
└── rust/
    ├── mod.rs                 # Enclave handler: API fetch + signing
    └── allowed_endpoints.yaml # Endpoint allowlist
```

Everything else (enclave runtime, attestation, key management) comes from the [Nautilus repo](https://github.com/GeunhwaJeong/nautilus). Your Move package depends on the enclave module:
...
## Wiring it into your own application

Adapting this to a different data source takes 4 changes:

1. **Match the structs.** The Rust response struct and the Move struct must have identical field names, in the same order, with compatible types. BCS serialization is order-sensitive, so a mismatch fails signature verification even when the signature is valid.
2. **Allowlist the domain.** Add your API host to `allowed_endpoints.yaml`, or the enclave cannot reach it.
3. **Set an intent constant.** The reference app uses `WEATHER_INTENT` with value `0` as a domain separator. Give each distinct message type its own value so a signature for one cannot be replayed as another.
4. **Register the enclave.** Publish your package, then call `GET /get_attestation` on the running enclave and submit the attestation to `register_enclave`. This stores the enclave public key onchain. The `init` function ships with placeholder PCR values; replace them with the real hashes from your built enclave image before Mainnet.

Once registered, call the enclave and submit its signed output:
...
The enclave returns the signed payload. Pass it to `update_weather` along with a reference to the `Enclave` object. The contract verifies the signature against the stored public key and aborts with `EInvalidSignature` if it does not match.
