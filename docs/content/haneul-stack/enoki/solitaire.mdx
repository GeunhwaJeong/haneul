...
title: Solitaire
description: >-
  Play solitaire onchain where every card shuffle uses Haneul's Random module and
  every move is a sponsored transaction verified on the blockchain.
keywords:
  - example
  - solitaire
  - game
...
goal:
  description: >-
    Reader can play solitaire onchain where every card shuffle uses Haneul's Random
    module
  requires:
    - has_frontmatter:
        - title
...
questions:
  - How do I build an onchain solitaire game on Haneul?
  - How does the solitaire example use onchain randomness?
  - What does the Enoki solitaire example demonstrate?
answer: >-
  Play solitaire onchain where every card shuffle uses Haneul's Random module and
  every move is a sponsored transaction verified on the blockchain.
---
...
```bash title='.env'
NEXT_PUBLIC_HANEUL_NETWORK=https://rpc.testnet.haneulfoundation.org:443
