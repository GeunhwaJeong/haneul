...
keywords: [ MVR, Move Registry, PackageInfo, metadata, source code, reverse resolution, TypeScript SDK, Haneul CLI ]
goal:
  description: >-
    Reader can create and manage PackageInfo objects, add source code
    information, and configure reverse resolution for Move packages
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I create a PackageInfo object in MVR?
  - How do I add source code information to a PackageInfo object?
  - How does reverse resolution work in MVR?
answer: >-
  Create a PackageInfo object by calling package_info::new with your UpgradeCap,
  then add source code info with git versioning, configure reverse resolution by
  setting a default metadata entry, and transfer the object to a safe address.
---
...
The TypeScript examples assume that you use the [MVR plugin](./tooling/typescript-sdk) for the Haneul TypeScript SDK. The Haneul CLI examples do not use the MVR plugin.
...
// call to publish and register in one step.)

/// Pass in the UpgradeCap
...
// Also create the visual representation of the "info" object.
...
// Set the display object to the info object.
...
transaction.moveCall({
  ...
  transaction.pure.string("<MVR name>"), // Example: @haneulns/core or haneulns.haneul/core
  ],
});
...
```
  ...
  <TabItem label="Haneul CLI" value='cli'>
  ```bash
haneul client ptb \
  ...
  --move-call haneul::tx_context::sender --assign sender \
...
Reverse resolution allows you to look up the human-readable name (like @haneulns/core or haneulns.haneul/core) associated with a given package ID, regardless of which version of the package you have. This is useful for identifying and verifying packages onchain, especially when working with tools or interfaces that reference package IDs directly.
...
```bash
curl --location 'https://mainnet.mvr.haneul-labs.com/v1/reverse-resolution/bulk' \
...
<Tabs groupId='language'>
  ...
  // 1. Create the `GitInfo` object.
  const git = transaction.moveCall({
  target: `@mvr/metadata::git::new`,
  arguments: [
  transaction.pure.string('<Your git repository, for example `https://github.com/GeunhwaJeong/mvr`>'),
  transaction.pure.string('<Your git subdirectory, for example `packages/mvr`>'),
  transaction.pure.string('<Your git commit hash or tag, for example `636d22d6bc4195afec9a1c0a8563b61fc813acfc`>'),
  ],
  });
  ...
  transaction.pure.u64(`<The version number for the given source code (for example 1)>`),
  git,
  ],
  });
  ```
  ...
  <TabItem label="Haneul CLI" value='cli'>
  ```bash
  haneul client ptb --move-call @mvr/metadata::git::new \
  "\"<Your git repository, for example https://github.com/GeunhwaJeong/mvr>\"" \
  \"<Your git subdirectory, for example packages/mvr>\" \
  \"<Your git commit hash or tag, for example 636d22d6bc4195afec9a1c0a8563b61fc813acfc>\" --assign git \
  --move-call @mvr/metadata::package_info::set_git_versioning @<Your PackageInfo object> \
  <The version number for the given source code (for example 1)> git
  ```
  </TabItem>
</Tabs>
...
You can transfer the `PackageInfo` object to another address (or object) by the following setup. If you want to build custom access control for your object, you can utilize a transfer to object workflow (see [Transfer to Object](/develop/objects/transfers/transfer-to-object) on the Haneul documentation site for more information).
...
<Tabs groupId='language'>
  ...
  <TabItem label="Haneul CLI" value='cli'>
  ```bash
  haneul client ptb --transfer-objects "[@<packageInfo>]" @<Your safe address>
  ```
  </TabItem>
</Tabs>