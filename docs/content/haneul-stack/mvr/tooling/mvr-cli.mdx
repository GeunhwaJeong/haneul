---
title: MVR CLI
description: Use the MVR CLI to manage Move package dependencies and interact with the Move Registry for building Move projects on Haneul.
keywords: [ MVR, Move Registry, CLI, dependencies, Move packages, installation, Cargo, binaries ]
goal:
  description: >-
    Reader can install the MVR CLI and use it to add dependencies and build
    Move projects with MVR package resolution
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I install the MVR CLI?
  - How do I add MVR dependencies to a Move project?
  - How do I build a Move project with MVR dependencies?
answer: >-
  Install the MVR CLI through Cargo, prebuilt binaries, or from source, then
  use mvr add to add named package dependencies to your Move.toml and
  haneul move build to resolve and build them.
---
...
```sh
cargo install --locked --git https://github.com/GeunhwaJeong/mvr --branch release mvr
```

### From binaries

Download the correct binary file for your OS and architecture from the [MVR GitHub release page](https://github.com/GeunhwaJeong/mvr/releases/latest).
...
1. Clone the `mvr` repository.
    ```sh
    git clone https://github.com/GeunhwaJeong/mvr.git
    ```
...
Use `haneul move build` as usual when building Move packages. The command automatically invokes MVR CLI to resolve dependencies and build your project.
