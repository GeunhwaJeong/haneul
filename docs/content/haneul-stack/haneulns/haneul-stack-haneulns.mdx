...
description: >-
  Integrate Haneul Name Service (HaneulNS) into your app using onchain or offchain
  resolution, reverse lookup, subnames, and the Enoki subname API, by walking
  through Haneul Messenger.
keywords:
  - HaneulNS
  - Haneul Name Service
  - domain names
...
goal:
  description: >-
    Reader can integrate Haneul Name Service (HaneulNS) into your app using onchain or
...
builder_paths:
  - path_id: p2p-payments
    ...
    step: HaneulNS integration
    stage: Payment Flow
questions:
  - What is HaneulNS and how does it work on Haneul?
  - How do I resolve HaneulNS domain names to Haneul addresses?
  - How do I use reverse lookup and subnames in an app?
answer: >-
  This guide explains how to integrate HaneulNS name resolution, reverse lookup,
...
- **Target address:** The address that a HaneulNS name points to. The NFT holder sets this. For example, `example.haneul` might point to `0x2`, making `0x2` the target address for `example.haneul`. Multiple names can point to the same target address.
- **Default address:** The HaneulNS name that the owner of a wallet address has designated to represent that address. For example, if you own `example.haneul` and its target address is your wallet, you can set `example.haneul` as the default name for your wallet. The owner must sign a set-default transaction to establish this connection.
...
For example, if `alice.haneul` points to address `0xA` and `0xA` has set `alice.haneul` as its default name:

1. The NFT holder changes the target address of `alice.haneul` from `0xA` to `0xB`.
2. The default name for `0xA` resets to empty. Reverse lookup for `0xA` now returns `null`.
3. `0xB` does not automatically inherit the default. The owner of `0xB` must sign a new set-default transaction to display `alice.haneul`.
...
A HaneulNS NFT (`HaneulnsRegistration`) acts as a capability object, not an identity token. The NFT grants the holder permission to:
...
- **`@haneullabs/haneul` client methods:** Every Haneul client provides `resolveNameServiceAddress` for name-to-address lookup and `defaultNameServiceName` for the reverse.
- **`@haneullabs/haneulns` SDK extension:** The HaneulNS SDK adds higher-level methods like `getNameRecord` to any Haneul client through the `$extend` pattern. Use this when you need name record details beyond the target address (expiration, metadata, avatar).

### Lookup with `@haneullabs/haneul`


{/* TODO: Import code from GeunhwaJeong/haneul-typescript — show resolveNameServiceAddress example */}
{/* Description: Basic name-to-address resolution using client.resolveNameServiceAddress */}

{/* TODO: Import code from GeunhwaJeong/haneul-typescript — show defaultNameServiceName example */}
...
For [GraphQL RPC](/develop/accessing-data/graphql/graphql-rpc), pass a name to the [`address`](/references/haneul-api/haneul-graphql/beta/reference/operations/queries/address) query for lookup, and read [`defaultNameRecord`](/references/haneul-api/haneul-graphql/beta/reference/types/objects/address) on the `Address` type for reverse lookup. To fetch a full name record, including expiration and metadata, use the [`nameRecord`](/references/haneul-api/haneul-graphql/beta/reference/operations/queries/name-record) query.

### Lookup with the HaneulNS SDK extension

The `@haneullabs/haneulns` package provides the `haneulns()` extension function. Attach it to any Haneul client through `$extend` to add HaneulNS methods. The extension uses `ClientWithCoreApi` and `PackageInfo` internally, and automatically loads the correct constants for Mainnet and Testnet:

<ImportContent source="src/haneulns-client.ts" mode="code" org="HaneulLabs" repo="haneulns-contracts" tag="haneulns-extension-example" language="ts" />

{/* TODO: If the haneulns-client.ts file does not have a docs:: tag for the extension example, import using the doc comment example at the top of the haneulns() function instead */}
...
When you resolve a HaneulNS name to send assets or authorize actions, verify 4 conditions before using the result. Skipping any check can result in assets sent to the wrong address, expired names, or unresolvable targets.


Follow this sequence for every onchain resolution:

1. **Verify the name exists.** Call `lookup` on the HaneulNS registry. If the result is `None`, the name is not registered or has been released after expiry. Abort the transaction.
...
{/* TODO: Import code from GeunhwaJeong/haneulns-contracts or a Haneul example repo — show offchain verification pattern using getNameRecord with expiry and target address checks */}
{/* Description: TypeScript function that resolves a HaneulNS name, validates expiry, and confirms target address before use */}

### Reverse lookup verification

When you display a name for an address in your UI, use reverse lookup (`defaultNameServiceName`) rather than scanning all names that point to that address. The reverse lookup returns only the name that the address owner has explicitly set as their default through a signed transaction. This guarantees that the address owner consented to being identified by that name ([HaneulNS developer reference](https://docs.haneulns.io/developer)).
...
- **HaneulNS portal ([haneulns.io](https://haneulns.io/)):** Create subnames manually through the web interface. Navigate to your domain, select the subnames tab, and create a new subname. You need to own the parent domain NFT in your connected wallet.
- **Enoki subname API:** Create subnames programmatically through a REST API. Best for apps that assign subnames to users automatically. See the [Enoki subname documentation](https://docs.enoki.haneul-labs.com/subnames) for API details.
- **Onchain through Move:** Create subnames directly in your smart contract using the HaneulNS subnames package. Use this when your subname creation logic is part of an onchain workflow.

##### Step 2:

Configure the subname with the following parameters:

- **Name:** The label for the subname (for example, `alice` for `alice.myapp.haneul`). Labels follow the same character rules as HaneulNS names: lowercase alphanumeric characters and hyphens, 3 to 63 characters.
...
If the parent domain expires, all subnames under it stop resolving, regardless of whether they are node or leaf subnames. Renew the parent domain at [haneulns.io](https://haneulns.io/) before the 30-day grace period ends to avoid losing subname resolution.
...
This example targets `@haneullabs/messaging` 0.3.0, which depends on `@haneullabs/haneul` 1.x and its `HaneulClient`. Do not mix it with the `HaneulGrpcClient` from `@haneullabs/haneul` 2.x. Check the versions your app resolves before combining these packages.

This pattern (composing Seal, Walrus, and Messaging onto a single Haneul client through `$extend`) is the standard way to build a multi-service Haneul Stack app. Each extension adds its methods to the client without affecting the others. `useUserSubname` then resolves the wallet's subname independently and the UI renders it next to messages and channel entries, replacing raw addresses throughout the app.
