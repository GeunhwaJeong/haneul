...
- **`@haneullabs/haneul` convenience methods:** The `HaneulClient` provides `resolveNameServiceAddress` and `resolveNameServiceNames` methods for basic lookups.
- **`@haneullabs/haneulns` SDK extension:** The HaneulNS SDK adds higher-level methods like `getNameRecord` to any `HaneulClient` through the `$extend` pattern. Use this when you need name record details beyond the target address (expiration, metadata, avatar).

### Lookup with `@haneullabs/haneul`


{/* TODO: Import code from GeunhwaJeong/haneul-typescript — show resolveNameServiceAddress example */}
{/* Description: Basic name-to-address resolution using HaneulClient.resolveNameServiceAddress */}

{/* TODO: Import code from GeunhwaJeong/haneul-typescript — show resolveNameServiceNames example */}
{/* Description: Reverse lookup (address to default name) using HaneulClient.resolveNameServiceNames */}

For [GraphQL RPC](/develop/accessing-data/graphql/graphql-rpc), use the `resolveHaneulnsAddress` query for lookup and the `defaultHaneulnsName` field on the `Address` type for reverse lookup. See the [Haneul GraphQL reference](https://docs.haneulfoundation.org/references/haneul-api/haneul-graphql/reference/api/queries/resolve-haneulns-address) for the full schema.

### Lookup with the HaneulNS SDK extension

The `@haneullabs/haneulns` package provides the `haneulns()` extension function. Attach it to any `HaneulClient` through `$extend` to add HaneulNS methods. The extension uses `ClientWithCoreApi` and `PackageInfo` internally, and automatically loads the correct constants for Mainnet and Testnet:
...
const extendedClient = new HaneulClient({ url: 'https://fullnode.testnet.haneulfoundation.org:443' })
  .$extend(
