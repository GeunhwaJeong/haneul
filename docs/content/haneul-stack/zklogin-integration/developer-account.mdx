...
description: >-
  zkLogin can be integrated with an application using an OpenID provider's OAuth
...
goal:
  description: >-
    Reader understands zkLogin can be integrated with an application using an
    OpenID provider's OAuth Client ID
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  - path_id: p2p-payments
    ...
    step: OpenID provider config
    stage: Auth
questions:
  - How do I configure openid providers?
  - How do I configure an openid provider?
  - What is Configure OpenID Providers in Haneul?
answer: >-
  zkLogin can be integrated with an application using an OpenID provider's OAuth
  Client ID and redirect URI.
---
...
When configuring OAuth clients, register the exact redirect URIs your app uses and avoid wildcard or open redirects in production, as these enable token interception. Use a separate OAuth client per environment (for example, local, Testnet, and Mainnet) so test credentials never reach production, and rotate client secrets periodically and whenever one might have been exposed. See [Security Best Practices](/develop/security/best-practices).
...
2. **Add all redirect URIs for the new domain.** Add every redirect URI your app uses on the new domain. Include the full path (for example, `https://newdomain.example.com/auth/callback`). Do not use wildcard URIs in production ([Security Best Practices](/develop/security/best-practices)).
...
4. **Update your Enoki Portal configuration (if applicable).** If you use the [Enoki platform](https://docs.enoki.haneul-labs.com/) for managed salt or proof generation, add the new domain to your Enoki app configuration. The Enoki salt service only accepts JWTs from allowlisted client IDs and origins.
