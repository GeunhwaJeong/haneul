...
This page is the technical reference for zkLogin. For a conceptual overview, see [zkLogin](/haneul-stack/zklogin-integration). For implementation steps, see the [Integration Guide](/haneul-stack/zklogin-integration/integration-guide).
...
2. **Salt service:** A backend service that returns a consistent, unique salt for each user. See [Salt management](/haneul-stack/zklogin-integration/integration-guide#step-4-manage-user-salt) for strategies.
...
A JSON data structure representing an OP's public keys. A public endpoint (for example, `https://www.googleapis.com/oauth2/v3/certs`) provides the keys corresponding to each `kid`. In Haneul, validators call JWK endpoints independently, and the latest JWKs for all supported providers are updated during protocol upgrades. Correctness is guaranteed by quorum (2f+1) of validator stake.
...
The zkLogin circuit and ceremony client [code](https://github.com/haneul-foundation/zk-ceremony-client) were open-sourced before the ceremony. An [audit report](https://github.com/haneul-foundation/security-audits/blob/main/docs/zksecurity_zklogin-circuits.pdf) on the circuit from zkSecurity was also published.
...
The resulting proving key is stored with the proving service. The verifying key was [deployed](https://github.com/GeunhwaJeong/haneul/pull/13822) as part of the validator software (protocol version 25 in [release 1.10.1](https://github.com/GeunhwaJeong/haneul/releases/tag/mainnet-v1.10.1)).
...
The salt is required for both zero-knowledge proof generation and address derivation. A leaked salt does not enable fund theft, but it allows an attacker to link the user's `sub` to their Haneul address. The impact depends on whether the provider uses pairwise identifiers (for example, Facebook, which is unique per app, low risk) or public reusable identifiers (for example, Google, Twitch, which have a globally unique `sub`, higher privacy risk).
...
By default, there is no link between the OAuth `sub` and the Haneul address (this is the purpose of the salt). The JWT is not published onchain. The values revealed onchain are `iss`, `aud`, and `kid` (needed to compute the public input hash). Sensitive fields like `sub` are private inputs to the proof.
...
1. **Haneul TypeScript SDK:** Use the [SDK](https://sdk.haneul-labs.com/typescript), which initializes a [GraphQL client](/references/haneul-graphql) under the hood.

2. **GraphQL endpoint:** Call `https://haneul-[network].haneul-labs.com/graphql` directly. See the <UnsafeLink href="/references/haneul-api/haneul-graphql/beta/reference/operations/queries/verify-zk-login-signature">GraphQL documentation</UnsafeLink>. Recommended if you do not want to run servers or handle JWK rotations.

3. **Haneul Keytool CLI** (debug use):

   ```sh
   haneul keytool zk-login-sig-verify --sig $ZKLOGIN_SIG --bytes $BYTES --intent-scope 3 --network devnet --curr-epoch 3
   ```

4. **Self-hosted verifier:** Use the [`zklogin-verifier`](https://github.com/GeunhwaJeong/zklogin-verifier) for custom logic.
...
Yes. You must produce a current JWT to use zkLogin. If recovery from a lost OAuth account is a concern, use a Haneul [multisig wallet](/develop/transactions/transaction-auth/multisig) with a backup signer, even another zkLogin signer from a different provider (for example, a 1-of-2 multisig with Google and Facebook).
