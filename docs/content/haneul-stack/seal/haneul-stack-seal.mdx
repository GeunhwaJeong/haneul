...
description: >-
  Learn how to encrypt Walrus blobs and enforce onchain access control using
  Seal, by walking through OnlyFins.
keywords:
  - seal
  - encryption
  - access control
...
goal:
  description: >-
    Reader can encrypt Walrus blobs and enforce onchain access control using
    Seal, by walking through OnlyFins
  requires:
    - has_frontmatter:
        - title
...
builder_paths:
  - path_id: walrus-general
    ...
    step: Store sensitive data (Seal)
    stage: Read & Verify
    eval: covered
  - path_id: walrus-agentic
    path_name: Walrus Agentic Data Storage
    step: Encrypt agent state (Seal)
    stage: Verify & Secure
    eval: covered
builder_paths_source:
  ...
  snapshot: '2026-07-10'
questions:
  - What is Encryption with Seal in Haneul?
  - How does encryption with seal work on Haneul?
  - Why should I use encryption with seal?
answer: >-
  Learn how to encrypt Walrus blobs and enforce onchain access control using
  Seal, by walking through OnlyFins.
---
...
:::caution
Seal should not be used to protect highly sensitive data such as wallet keys, personal health information, or government-level secrets. Review the [Seal Terms of Service](/haneul-stack/seal/terms-of-service) and [security best practices](/haneul-stack/seal/security-best-practices) before deploying to production.
:::
...
The key server object IDs come from `constants.ts`. These IDs point to onchain `KeyServer` objects that always hold the latest URL as the source of truth. See the [Seal documentation](/haneul-stack/seal/pricing) for the full list of verified key servers and their modes (open vs. permissioned).
...
:::tip
Seal supports envelope encryption for large files. Generate a symmetric key, encrypt the image data with AES, then encrypt only the symmetric key with Seal. This is more efficient for large payloads and is the recommended pattern for anything larger than a few hundred KB. See the [Seal documentation](/haneul-stack/seal/using-seal) for details.
:::
