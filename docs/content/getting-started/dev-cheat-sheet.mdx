...
goal:
  description: >-
    Reader can quickly look up best practices for Move, app development,
    signing, and zkLogin
  requires:
    - min_words: 500
      label: Has enough entries to be useful
    - pattern: '\[.*\]\(/.*\)'
      min: 3
...
questions:
  - What is Developer Cheat Sheet in Haneul?
  - How do I sign?
  - How does developer cheat sheet work on Haneul?
answer: Quick reference on best practices for Haneul Network developers.
---
...
- Use the [capability pattern](https://github.com/GeunhwaJeong/haneul-move-bootcamp/tree/solution/C1/capability) for access control instead of address checks. Define capability objects like `AdminCap` or `MintCap` with the `key` ability, require them as function parameters for privileged operations, transfer them to delegate access, and destroy them to revoke access. This is more composable than `tx_context::sender()` checks because other modules can hold and pass capabilities on a user's behalf.
...
- Use the [`std::unit_test`](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-framework/packages/move-stdlib/sources/unit_test.move) module for `assert_eq!` and `assert_ref_eq!` macros and the `destroy` function for cleaning up objects in tests.
...
- Use `haneul move lint` to run the Move linter on your package and catch common issues and anti-patterns before publishing.
