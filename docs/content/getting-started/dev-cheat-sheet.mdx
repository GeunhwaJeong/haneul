...
- Use the [capability pattern](/getting-started/examples/capability-pattern) for access control instead of address checks. Define capability objects like `AdminCap` or `MintCap` with the `key` ability, require them as function parameters for privileged operations, transfer them to delegate access, and destroy them to revoke access. This is more composable than `tx_context::sender()` checks because other modules can hold and pass capabilities on a user's behalf.
