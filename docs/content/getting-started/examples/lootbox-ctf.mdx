...
title: Lootbox
description: >-
  Exploit a public randomness function in a lootbox contract by deploying a
  wrapper module that aborts losing transactions, so you only pay when you win.
keywords:
  - example
  - lootbox
  - randomness
...
goal:
  description: >-
    Reader can exploit a public randomness function in a lootbox contract by
    deploying a wrapper module that aborts
  requires:
    - headings:
...
questions:
  - How do I build a lootbox system on Haneul?
  - What does the lootbox CTF example demonstrate?
  - How do I use randomness for loot drops in Move?
answer: >-
  Exploit a public randomness function in a lootbox contract by deploying a
  wrapper module that aborts losing transactions, so you only pay when you win.
---
...
Fund the generated address with Testnet HANEUL and USDC. For Testnet HANEUL, visit the [HANEUL Faucet](https://faucet.haneulfoundation.org/). For USDC, use a faucet such as [Circle's USDC faucet](https://faucet.circle.com/) and select **Haneul Testnet** as the network.
