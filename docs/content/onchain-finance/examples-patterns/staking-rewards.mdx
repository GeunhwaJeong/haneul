...
```bash
$ git clone https://github.com/GeunhwaJeong/staking-contract-template.git
...
The package uses implicit framework dependencies (empty `[dependencies]` in `Move.toml`), so the correct Haneul framework version resolves from the active environment automatically. Take note of your package ID and the `StakingPool` and `AdminCap` object IDs from the output.

## Run the example

Every entry point is a `public fun` that returns its objects instead of transferring them internally. This makes the module composable in programmable transaction blocks, but it also means plain `haneul client call` fails with `UnusedValueWithoutDrop`. Use `haneul client ptb` and transfer the results explicitly.
...
**Fix:** Use `haneul client ptb` and explicitly transfer the returned objects. See the [Run the example](#run-the-example) section for the correct syntax.
