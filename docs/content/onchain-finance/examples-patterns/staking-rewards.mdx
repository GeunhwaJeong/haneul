...
repo: https://github.com/GeunhwaJeong/staking-contract-template
path: /staking
concepts:
  - reward_accumulator
  - shared_objects
...
- [x] [Create an address and fund it with HANEUL](/getting-started/onboarding/get-coins) for gas.
...
A complete, tested implementation lives in [`staking-contract-template`](https://github.com/GeunhwaJeong/staking-contract-template). Build it, run its tests, and publish it as-is, or lift the accumulator logic into a package of your own:

```bash
$ haneul client publish --gas-budget 100000000
```

The package leaves `[dependencies]` empty in `Move.toml`, so the framework version resolves from your active environment. Record the package ID and the `StakingPool` and `AdminCap` object IDs from the output.

## Calling the contract

Every entry point is a `public fun` that returns its objects instead of transferring them internally. That makes the module composable in programmable transaction blocks, but it also means a plain `haneul client call` fails with `UnusedValueWithoutDrop`. Use `haneul client ptb` and transfer the results explicitly.
...
Fund the pool with reward tokens and start the emission window. `DURATION_MS` is the emission window length in milliseconds.
...
Fully exit the position. The function consumes the receipt and returns principal plus accrued rewards.
...
<CodeWalkthrough source="staking/sources/staking.move" org="HaneulLabs" repo="staking-contract-template" language="move">
...
For a hands-on exercise exploring what happens when a staking contract gets the accumulator pattern wrong, see the [staking challenge in the CTF repository](https://github.com/GeunhwaJeong/CTF).
...
**Symptom:** `haneul client call` fails with `UnusedValueWithoutDrop`.

**Cause:** Every public function returns objects instead of transferring them. Plain `haneul client call` does not consume the return values.

**Fix:** Use `haneul client ptb` and explicitly transfer the returned objects. See [Calling the contract](#calling-the-contract) for the correct syntax.
