...
description: >-
  ...
  movement on Haneul through Accounts, Policies, and programmable approval logic.
keywords:
  - PAS
...
goal:
  description: >-
    Reader can learn how the Permissioned Asset Standard (PAS) enforces
    restricted asset movement on Haneul through Accounts
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Permissioned Asset Standard in Haneul?
  - How does permissioned asset standard work on Haneul?
  - Why should I use permissioned asset standard?
answer: >-
  On Haneul, any object with the `store` ability can be freely transferred by its
  owner. `Balance` and `Coin` both have `store`, meaning if you hold them, you
  can send them anywhere on the network with no restrictions.
---
