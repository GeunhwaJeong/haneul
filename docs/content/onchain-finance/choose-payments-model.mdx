---
  ...
  description: >-
    Compare basic transfers and Payment Kit for Haneul payment integrations, learn
  ...
  goal:                                                                         
    description: >-
      ...
      their Haneul payment integration.          
    requires:                                                                   
      - has_frontmatter:
          - title                                                               
          - description                                                      
  ...
  questions:                              
    - How do I choose a payment model on Haneul?
    - What is the difference between basic transfer and Payment Kit on Haneul?     
    - Should I use address balances or coin objects for payments?
    - How do I pay gas for a payment transaction on Haneul?                        
    - What is a gasless stablecoin transfer on Haneul?                          
    - How do sponsored transactions work for Haneul payments?
  answer: >-                                  
    Haneul supports two main payment models out of the box. Basic transfers move   
...
Percent-encode every parameter value according to RFC 3986. The `amount` value is the native amount of the coin type, so 1000000000 GEUNHWA represents 1 HANEUL.
...
- **Integer math only:** HANEUL has 9 decimals (1 HANEUL = 1,000,000,000 GEUNHWA). Never use floating-point arithmetic on token amounts. Always work in the smallest unit, GEUNHWA for HANEUL, and convert for display only.
- **Per-coin-type decimals:** Not all tokens use 9 decimals. USDC on Haneul has 6 decimals. Always check `CoinMetadata.decimals` for the specific coin type before you convert between human-readable and onchain amounts.
...
- **Gas coin reservation:** If you pay gas in HANEUL and also transfer HANEUL in the same transaction, the transaction implicitly uses the gas coin for gas. Split the transfer amount from the gas coin rather than selecting a separate HANEUL coin object, which avoids `insufficient gas` errors.
...
The sender pays in HANEUL. This option takes the least work to implement, but your users must hold HANEUL. See [Gas Fees](/develop/transaction-payment/gas-in-haneul).

### Gasless stablecoin transfer

For allowlisted stablecoins sent through `balance::send_funds`, the network waives gas fees entirely, so the sender needs no HANEUL. Protocol configuration governs the allowlist, which currently includes USDC, USDhaneul, USDY, FDUSD, AUSD, USDB, and Ethena USDe. A transaction qualifies only when all of the following hold:
...
Build the transfer with `HaneulGrpcClient`, which detects eligibility during simulation and sets the gas price and budget to 0 for you:
...
const client = new HaneulGrpcClient({
	network: 'mainnet',
	baseUrl: 'https://fullnode.mainnet.haneulfoundation.org:443',
});
