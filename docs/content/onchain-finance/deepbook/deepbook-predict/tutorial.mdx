...
answer: >-
  ...
  @haneullabs/deepbook-v3 SDK, which carries the Mainnet and Testnet deployments and
...
- [x] Node.js 22 or later, with `@haneullabs/deepbook-v3` 2.3.0 or later and its `@haneullabs/haneul` 2.30.0 or later peer dependency installed. See the [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript) for client setup.
- [x] A Testnet address funded with HANEUL for gas from a [Haneul Testnet faucet](/getting-started/onboarding/get-coins). Hold at least 1 HANEUL as a working reserve.
- [x] Test USDC from the [DeepBook Predict Testnet token request form](https://tally.so/r/Xx102L). Wallets display it as DUSDC. It never pays gas, so you need HANEUL as well. On Mainnet the quote asset is native USDC and there is no faucet.
...
|---------|----------|--------------|
| Predict | `https://predict-server-v4.testnet.haneul-labs.com` | Markets, market state, positions, vault state, protocol events, indexed configuration |
...
```bash
$ curl 'https://predict-server-v4.testnet.haneul-labs.com/markets?active=true&limit=50'
$ MARKET_ID=0x...
$ curl "https://predict-server-v4.testnet.haneul-labs.com/markets/${MARKET_ID}/state"
$ curl "https://predict-server-v4.testnet.haneul-labs.com/markets/${MARKET_ID}/open-interest"
```
...
```bash
$ curl "https://propbook-server-v4.testnet.haneul-labs.com/oracle-bindings"
$ PYTH=0x... VALUES=0x... SVI=0x...
$ curl "https://propbook-server-v4.testnet.haneul-labs.com/oracles/${PYTH}/pyth/latest"
$ curl "https://propbook-server-v4.testnet.haneul-labs.com/oracles/${VALUES}/block-scholes/spot?limit=10"
$ curl "https://propbook-server-v4.testnet.haneul-labs.com/oracles/${VALUES}/block-scholes/forward?limit=10"
$ curl "https://propbook-server-v4.testnet.haneul-labs.com/oracles/${SVI}/block-scholes/svi?limit=10"
```
...
```bash
$ ACCOUNT_ID=0x...
$ curl "https://account-server-v4.testnet.haneul-labs.com/accounts/${ACCOUNT_ID}/portfolio"
$ curl "https://account-server-v4.testnet.haneul-labs.com/accounts/${ACCOUNT_ID}/balances"
$ curl "https://account-server-v4.testnet.haneul-labs.com/accounts/${ACCOUNT_ID}/activity"
$ curl "https://predict-server-v4.testnet.haneul-labs.com/accounts/${ACCOUNT_ID}/positions?status=open&limit=100"
```
...
1. Fund a Testnet address with HANEUL, then confirm the balance with `haneul client gas`.
1. Request test USDC, then confirm the balance with `haneul client balance`, where it appears as DUSDC.
...
1. Create the account, then confirm `client.predict.wrapperIdFor(owner)` resolves with `haneul client object WRAPPER_ID`.
