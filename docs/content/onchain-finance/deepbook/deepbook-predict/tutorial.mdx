...
description: >-
  End-to-end DeepBook Predict walkthrough on Haneul Testnet: oracle context,
...
answer: >-
  This tutorial walks through a full DeepBook Predict flow on Haneul Testnet:
...
A complete DeepBook Predict flow on Haneul Testnet covers reading a live oracle, minting binary and vertical range positions, redeeming before and after settlement, and running the liquidity provider vault flow. It continues from the [DeepBook Predict quickstart](/onchain-finance/deepbook/deepbook-predict#quickstart-mint-a-binary-position), which covers install, funding, and your first mint.

There is no dedicated Predict TypeScript SDK. You build every transaction with the [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript) against the protocol's Move entry points. The transaction samples live in the `examples/deepbook-predict` package, which CI type-checks with `tsc --noEmit` against `@haneullabs/haneul` version 2.22.1. This document does not execute them against Testnet. The oracle-list response shape matches the live Testnet server. Run the [manual verification steps](#verify-on-testnet) before you rely on the write paths.
...
- [x] [Haneul TypeScript SDK](https://sdk.haneul-labs.com/typescript) installed (`npm install @haneullabs/haneul`).
- [x] A Testnet address funded with HANEUL for gas from a [Haneul Testnet faucet](/getting-started/onboarding/get-coins). Hold at least 1 HANEUL as a working reserve, and see [How much HANEUL to hold](/onchain-finance/deepbook/deepbook-predict#how-much-haneul-to-hold) for sizing gas budgets.
- [x] DUSDC from the [Predict token request form](https://tally.so/r/Xx102L). DUSDC does not pay gas, so you need HANEUL as well.
...
Fetch the `Predict` object with its contents, then read 4 fields. The Predict server itself reads exactly these paths in [`crates/predict-server/src/server.rs`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/crates/predict-server/src/server.rs), so treat them as the reference layout:
...
The Predict oracle feed service already does this, in [`simulateReturnValues`](https://github.com/GeunhwaJeong/deepbookv3/blob/predict-testnet-4-16/scripts/services/oracle-feed/chain.ts#L178-L196). Follow its 3 choices, because each one matters for read calls:
...
1. Fund a Testnet address with HANEUL, then confirm with `haneul client gas`.
2. Request DUSDC, then confirm your DUSDC balance with `haneul client balance`.
3. Fetch a live oracle with `getActiveOracle`. Confirm the JSON includes an oracle with `"status": "active"`.
4. Create a `PredictManager` (see the quickstart) and confirm the ID resolves with `haneul client object MANAGER_ID`.
...
8. Run `supplyLiquidity`, then confirm you hold a `PLP` coin with `haneul client objects`.
