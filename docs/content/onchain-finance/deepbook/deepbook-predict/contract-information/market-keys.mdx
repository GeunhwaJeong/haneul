...
Pick a vertical range for a view that settlement lands inside a band. A range wins on a bounded outcome, so it costs less per contract and wins less often. Ranges haneult a view on where a price settles rather than which direction it moves.
...
Both types carry `copy`, `drop`, and `store`, the exact abilities Haneul requires of a `Table` key. That is why the protocol can store positions as table rows instead of as objects:
...
Two keys are equal when every field matches. Move compares them structurally with `==`, and Haneul tables compare the BCS serialization of the key, which follows declaration order. For `MarketKey` that order is `oracle_id`, `expiry`, `strike`, `direction`.
