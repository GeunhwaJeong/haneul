...
Both functions abort with `EInvalidOwner` when the sender is not the manager owner. Neither takes a `ctx` argument in the transaction block, because Haneul supplies `TxContext` automatically.
