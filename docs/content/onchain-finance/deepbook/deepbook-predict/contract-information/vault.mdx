...
One market per transaction is the intended cadence. Only `value_expiry` walks a payout tree, and a tree holds one dynamic-field child per distinct strike tick, so splitting the walk across transactions is what keeps a large book inside Haneul's per-transaction object budget.
...
`@haneullabs/deepbook-v3` version 2.3.0 exposes the LP path on its `/predict` subpath for both networks. Every read runs against the Haneul client's core API, with no indexer or server in the path.
