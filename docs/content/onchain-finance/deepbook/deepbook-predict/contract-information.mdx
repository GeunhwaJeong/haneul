...
questions:
  - Which package and object IDs does DeepBook Predict use on Haneul Mainnet and Testnet?
...
answer: >-
  DeepBook Predict runs as the deepbook-predict-mainnet deployment on Haneul
  Mainnet and the deepbook-predict-testnet deployment on Haneul Testnet. Each
  ...
  and 5m cadences. The @haneullabs/deepbook-v3 SDK ships both deployments and reads
  every value without an indexer.
---

DeepBook Predict runs on Haneul Mainnet and Haneul Testnet as 2 separate deployments, `deepbook-predict-mainnet` and `deepbook-predict-testnet`. Version 2.3.0 or later of [`@haneullabs/deepbook-v3`](https://www.npmjs.com/package/@haneullabs/deepbook-v3) carries both, so `getConfig('mainnet')` and `getConfig('testnet')` each return the identifiers on this page. Both deployments build from the [`deepbook-predict-mainnet` branch](https://github.com/GeunhwaJeong/deepbookv3/tree/deepbook-predict-mainnet/packages/predict) of the DeepBookV3 repository. The Testnet sources are identical to the Mainnet source commit apart from 2 lines of documentation comment and attribute in `packages/propbook/sources/feeds/pyth_feed.move`, so one source pin serves both networks.
...
| Source branch | [`deepbook-predict-mainnet`](https://github.com/GeunhwaJeong/deepbookv3/tree/deepbook-predict-mainnet/packages/predict) |
...
| Source branch | [`deepbook-predict-testnet`](https://github.com/GeunhwaJeong/deepbookv3/tree/deepbook-predict-testnet/packages/predict) |
...
| --- | --- | --- |
| Predict | `https://predict-server-v4.testnet.haneul-labs.com` | Markets, market state, positions, vault state, protocol events, and indexed configuration |
| Propbook | `https://propbook-server-v4.testnet.haneul-labs.com` | Oracle bindings, Pyth observations, and Block Scholes spot, forward, and SVI observations |
| Account | `https://account-server-v4.testnet.haneul-labs.com` | Account custody state, balances, activity, portfolio, and app authorizations |
...
```bash
$ curl 'https://predict-server-v4.testnet.haneul-labs.com/markets?active=true&limit=1'
```
...
```bash
$ curl 'https://predict-server-v4.testnet.haneul-labs.com/status'
$ curl 'https://propbook-server-v4.testnet.haneul-labs.com/status'
$ curl 'https://account-server-v4.testnet.haneul-labs.com/status'
```
...
```ts
import { getConfig, getDeployment } from '@haneullabs/deepbook-v3/predict';
...
- **Quote asset and funding:** Testnet quotes in a test coin you can request for free, and Testnet HANEUL is free. Mainnet quotes in native USDC, there is no faucet, and any top-up path that assumes free replenishment needs replacing.
...
Stream Predict events with `SubscriptionService.SubscribeEvents`, filtering on the Predict package ID, and page the same events historically with `LedgerService.ListEvents` under an identical filter. Haneul full nodes no longer serve the older WebSocket subscription API, so do not build on `haneul_subscribeEvent`.

Subscribe to market creation and settlement on Testnet. For Mainnet, swap in the Mainnet Predict package ID and `fullnode.mainnet.haneul.io:443`:

```shell
$ PACKAGE=0x25d075d2de915feda7ab9b8f855afe59cbcd4b2ce96f75324940c810f84da018
$ ENDPOINT=fullnode.testnet.haneul.io:443
$ METHOD=haneul.rpc.v2.SubscriptionService/SubscribeEvents
...
Swap the method for `haneul.rpc.v2.LedgerService/ListEvents` to page the same events historically. The filter is disjunctive normal form, so the server ORs the `terms` blocks: the request above matches either event type and scopes both to one deployment.
...
The links below pin every module to the Mainnet source commit `14a7e8f822e0397df2d61fbfbce3ef21086891c2`, which the [`deepbook-predict-mainnet`](https://github.com/GeunhwaJeong/deepbookv3/tree/deepbook-predict-mainnet) branch carries. The Testnet deployment's sources are identical apart from 2 lines in `pyth_feed.move`, so the same links serve both networks. There is no `entry fun` in any Predict package: every integrator call is a `public fun` invoked as a `moveCall`, and several of them return values that a later command in the same transaction must consume.

| **Area** | **Source** |
| --- | --- |
| Markets, quoting, trading, and settlement | [`packages/predict/sources/expiry_market.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/predict/sources/expiry_market.move) |
| Market catalog, cadences, and capabilities | [`packages/predict/sources/registry/registry.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/predict/sources/registry/registry.move) |
| Protocol configuration and gating | [`packages/predict/sources/config/protocol_config.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/predict/sources/config/protocol_config.move) |
| PLP pool, liquidity queues, and valuation | [`packages/predict/sources/plp/plp.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/predict/sources/plp/plp.move) |
| Pool valuation capability | [`packages/predict/sources/capabilities/pool_valuation_cap.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/predict/sources/capabilities/pool_valuation_cap.move) |
| Account custody and authorization | [`packages/account/sources/account.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/account/sources/account.move) |
| Oracle registry and bindings | [`packages/propbook/sources/registry.move`](https://github.com/GeunhwaJeong/deepbookv3/blob/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/propbook/sources/registry.move) |
| Pyth and Block Scholes feeds | [`packages/propbook/sources/feeds`](https://github.com/GeunhwaJeong/deepbookv3/tree/14a7e8f822e0397df2d61fbfbce3ef21086891c2/packages/propbook/sources/feeds) |
