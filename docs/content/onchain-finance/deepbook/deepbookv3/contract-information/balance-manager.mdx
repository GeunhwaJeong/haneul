...
keywords:
  ...
  - tradeproof
goal:
  description: Reader can learn about the balance manager and how it works
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  - path_id: defi-deepbook
    ...
    step: Balance manager
    stage: DeFi Primitives
questions:
  - What is BalanceManager in Haneul?
  - How does balancemanager work on Haneul?
  - Why should I use balancemanager?
answer: Learn about the balance manager and how it works.
---

The `BalanceManager` shared object holds all balances for different assets. To perform trades, pass a combination of `BalanceManager` and `TradeProof` into a [pool](/onchain-finance/deepbook/deepbookv3/design#pool). `TradeProof`s are generated in one of two ways, either by the `BalanceManager` owner directly, or by any `TradeCap` owner. The owner can generate a `TradeProof` without the risk of equivocation. The `TradeCap` owner, because it's an owned object, risks equivocation when generating a `TradeProof`. Generally, a high frequency trading engine trades as the default owner.
...
The system emits this event when you create a new balance manager.
...
The system emits this event when a deposit or withdrawal occurs.
