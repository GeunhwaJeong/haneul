...
description: >-
  ...
  and staking fees, and obtaining Testnet DEEP, USDC, and HANEUL.
keywords:
  - deepbook fees
  - deep token
  - pay_with_deep
...
answer: >-
  ...
  form, or swap faucet HANEUL for DEEP when the DEEP_HANEUL book has liquidity.
builder_paths:
...
The TypeScript samples on this page live in the `examples/deepbook-spot` package and type-check with `tsc --noEmit` against `@haneullabs/deepbook-v3` and `@haneullabs/haneul`. They reference pools and coins by SDK key (for example `DEEP_HANEUL`, `HANEUL_DBUSDC`, `DEEP`) rather than hardcoding onchain IDs. See the [DeepBookV3 SDK constants](/onchain-finance/deepbook/deepbookv3-sdk#keys) for the key-to-ID mapping and [Contract Information](/onchain-finance/deepbook/deepbookv3/contract-information) for the canonical IDs.
...
<ImportContent source="packages/deepbook/sources/state/trade_params.move" mode="code" org="HaneulLabs" repo="deepbookv3" fun="taker_fee_for_user" noComments />
...
## Obtain Testnet DEEP, USDC, and HANEUL
...
- **Swap HANEUL for DEEP (when the book has liquidity):** on the whitelisted `DEEP_HANEUL` pool, faucet HANEUL converts to DEEP with no BalanceManager. The Testnet `DEEP_HANEUL` book is thin and intermittent, so treat this as a convenience, not a guarantee.

<Tabs className="tabsHeadingCentered--small">
<TabItem value="prereq" label="Prerequisites">
- [x] A current CLI, updated with `haneulup update`: version skew between your CLI and the network produces confusing failures. Confirm the client and server API versions match with `haneul --version`. See [Install Haneul](/references/cli/client).
- [x] An active environment on a current Testnet RPC: check with `haneul client active-env` and `haneul client envs`, because a stale or wrong RPC URL reads empty balances and rejects transactions. See [Haneul environment setup](/references/contribute/haneul-environment).
</TabItem>
</Tabs>

##### Step 1: Get Testnet HANEUL

Find your address with `haneul client active-address`, then check your balance with `haneul client gas` before requesting anything. Faucets are rate-limited per address and IP, so request only when the balance is actually empty. Request Testnet HANEUL from a [Haneul faucet](/getting-started/onboarding/get-coins), which lists the browser, Discord, and cURL routes. If the primary faucet rate-limits you, use one of those fallback routes.
...
If the `DEEP_HANEUL` book has liquidity, you can instead swap faucet HANEUL for DEEP with no BalanceManager, which avoids a chicken-and-egg where you would need DEEP to buy DEEP. Check the quote first with `getQuantityOut`: if it returns fewer DEEP than the pool's minimum size (10 DEEP), the book is too thin, so use the form. Set a nonzero `minOut` so a book that empties reverts instead of returning your HANEUL unfilled:
...
- `InsufficientCoinBalance`: the transaction spends more of a coin than you hold after gas. The swap draws HANEUL from the same balance that pays gas, so size it below your HANEUL balance minus a gas budget, or fund more HANEUL. The SDK also reserves a default gas budget when you do not set one, so leave room for it or call `setGasBudget` explicitly and size the swap around it.
...
- **Swap succeeds but you receive zero DEEP:** the book was empty and `minOut` was 0, so the pool returned your HANEUL unfilled. Set a nonzero `minOut` from `getQuantityOut` so an empty book reverts, and check the quote before swapping.
