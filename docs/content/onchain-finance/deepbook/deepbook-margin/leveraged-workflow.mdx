...
description: >-
  Hands-on DeepBook Margin walkthrough on Haneul Testnet: create a MarginManager,
...
This walkthrough uses the `HANEUL_DBUSDC` pool, borrowing the stable quote asset (DBUSDC) against a volatile base collateral (HANEUL) to open a leveraged long on HANEUL. Health monitoring here is entirely oracle-priced, so the pair choice matters: on Testnet the HANEUL and USDC price feeds are canonical Pyth feeds, whereas the Testnet DEEP feed is a substitute (an HFT feed) rather than a dedicated DEEP oracle. Do not build oracle-priced risk math on the substitute feed.

The samples live in the `examples/deepbook-margin` package, type-checked with `tsc --noEmit` against `@haneullabs/deepbook-v3` and `@haneullabs/haneul`, and reference pools, coins, and margin pools by SDK key rather than hardcoding onchain IDs.

<Tabs className="tabsHeadingCentered--small">
<TabItem value="prereq" label="Prerequisites">
- [x] A current CLI: run `haneulup update haneul`, then `haneul --version` to confirm it.
- [x] Testnet HANEUL on your address for collateral and gas, from the [Haneul faucet](/getting-started/onboarding/get-coins).
- [x] For the borrow-and-trade steps, enough HANEUL for collateral and gas, plus DBUSDC borrow liquidity in the margin pool. You supply HANEUL as collateral and borrow DBUSDC from the pool, so you do not need DBUSDC in your wallet.
- [x] The SDK installed: `npm install @haneullabs/deepbook-v3 @haneullabs/haneul`.
</TabItem>
</Tabs>
...
**Observe:** `borrowableNow` is positive for the asset you intend to borrow. For the DBUSDC pool on Testnet at the time of writing, supply is about `10087` and borrow about `119`, so headroom is ample; the HANEUL pool runs at higher utilization with less headroom and a higher rate. If `borrowableNow` is at or near zero, borrow a smaller amount, wait for supply, or borrow the other asset.

:::info

The remaining steps borrow real funds: they supply HANEUL as collateral and borrow DBUSDC from the margin pool, so you need HANEUL for collateral and gas and enough DBUSDC borrow liquidity in the pool, not DBUSDC in your wallet. They appear as type-checked samples with the observable result to confirm when you run them. The code is identical to what an executed run uses, so once the pool has borrow liquidity you run each step in order and check the stated result.

:::

## Deposit collateral

Deposit collateral into the manager. Collateral backs your borrow: the more you deposit before borrowing, the higher your starting risk ratio. This walkthrough deposits HANEUL as base collateral.

<ImportContent source="examples/deepbook-margin/src/collateral.ts" mode="code" tag="deposit-collateral" />

Size the deposit to your wallet balance, and keep a HANEUL reserve for gas:
...
With borrowed DBUSDC in the manager, place a bid on `HANEUL_DBUSDC` to buy HANEUL. Margin orders go through the pool proxy, not the spot order entry, so the borrow and the trade stay bound to the manager. The `clientOrderId` is your own tag: the SDK types it as a string, but the contract encodes it as `u64`, so pass a numeric string such as a timestamp, not a label. Price and quantity must respect the pool's tick and lot size.

<ImportContent source="examples/deepbook-margin/src/place-order.ts" mode="code" tag="open-position" />

**Observe:** the order rests or fills against the book, and your position in HANEUL is now larger than your collateral alone could fund. The borrow funds that surplus, which is the leverage.
...
**Observe:** `riskRatio` sits above `liquidationRiskRatio`, and `marginToLiquidation` is the cushion you have. Because the ratio is oracle-priced, it moves whenever HANEUL moves, and it drifts down on its own as interest accrues, even if the price is flat.
...
<ImportContent source="packages/deepbook_margin/sources/helper/margin_constants.move" mode="code" org="HaneulLabs" repo="deepbookv3" lines="25-28" />
...
<ImportContent source="packages/margin_liquidation/sources/liquidation_vault.move" mode="code" org="HaneulLabs" repo="deepbookv3" fun="liquidate_base" noComments />
...
**Observe:** after the reduce-only order fills, your HANEUL position shrinks; after repay, the manager's borrowed shares drop to zero (or by the repaid amount) and `getMarginManagerState('MM').riskRatio` returns to its maximum once debt is cleared.
