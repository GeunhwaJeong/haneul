...
description: >-
  Resolve DeFi outcomes on Haneul: price-based settlement for parametric outcomes
  with a Move resolver, and an optimistic dispute pattern for subjective
  outcomes, expressed as an illustrative interface with no canonical Haneul
  protocol.
keywords:
  - resolution
  - settlement
...
questions:
  - How do I resolve a prediction market on Haneul?
  - How do I settle an outcome against a price?
  - Is there an optimistic oracle on Haneul?
...
answer: >-
  ...
  exists on Haneul today, so that pattern is something you build.
builder_paths:
...
No canonical optimistic-oracle protocol exists on Haneul as of this writing. UMA's Optimistic Oracle, the reference design that resolves Polymarket, is an EVM protocol with no Haneul Move equivalent. The interface below is an illustrative pattern to build against your own governance, not a deployable dependency you can import.

:::

A minimal optimistic resolver on Haneul is a state machine over proposal, bond, dispute, and escalation:

```move
// ILLUSTRATIVE PATTERN, NOT A DEPLOYABLE PACKAGE. Sketch of an optimistic
// resolver you would implement and audit yourself; no canonical Haneul protocol
// provides this.
