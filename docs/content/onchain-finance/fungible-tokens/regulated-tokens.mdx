...
description: >-
  Create regulated currencies on Haneul using the Coin Registry system with deny
  list capabilities for access control.
keywords:
  - regulated coin list
  - regulated coins
  - regulate coin
  ...
  - create a token
  - how to create token
  - HANEUL token standards
  - coin registry
goal:
  description: >-
    Reader can create regulated currencies on Haneul using the Coin Registry system
    with deny list capabilities for access control
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  - path_id: p2p-payments
    ...
    step: Regulated tokens
    stage: Tokens
questions:
  - What is Regulated Currencies in Haneul?
  - How do I create regulated currency?
  - How does regulated currencies work on Haneul?
answer: >-
  Regulated currencies are assets with access control managed through a **deny
  list**. A deny list identifies addresses that cannot send or receive that
  asset. For example, stablecoins are a type of regulated currency.
---
...
| `HANEUL_FULLNODE_URL` | The URL to the full node network that processes transactions. For Testnet this value is `https://fullnode.testnet.haneulfoundation.org:443`. |
