...
description: >-
  Tokens do not allow transfers, conversions, or spends by default. To allow
  these actions, use an `ActionRequest`.
keywords:
  - action request
  - protected actions
  - action
...
goal:
  description: >-
    Reader understands how an ActionRequest authorizes protected token actions
    and how to confirm one
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Action Request in Haneul?
  - How does action request work on Haneul?
  - Why should I use action request?
answer: >-
  Tokens do not allow transfers, conversions, or spends by default. To allow
  these actions, use an `ActionRequest`.
---
...
An example of a transaction implemented in TypeScript with `@haneullabs/haneul`, confirming an action request with a `TreasuryCap`. Here the admin account owns the `TreasuryCap`, which is used to mint and confirm the transfer request for the token:
