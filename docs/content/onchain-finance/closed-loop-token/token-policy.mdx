...
title: Token Policy
description: >-
  A `TokenPolicy` is a shared object that defines a token's allowed actions and
  conditions. Wallets and other financial services often use `TokenPolicy`
  objects.
keywords:
  - token policy
  - tokenpolicy
...
goal:
  description: >-
    Reader understands a `TokenPolicy` is a shared object that defines a token's
...
questions:
  - What is Token Policy in Haneul?
  - How do I create and share a TokenPolicy?
  - What rules can I add to a TokenPolicy?
answer: >-
  A `TokenPolicy` is a shared object that defines a token's allowed actions and
  conditions. Wallets and other financial services often use `TokenPolicy`
  objects.
