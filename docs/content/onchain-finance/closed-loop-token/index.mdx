...
description: >-
  Closed-Loop tokens can only be used for a specific service or by authorized
  users.
keywords:
  - 'token::keep'
  - 'token::join'
...
goal:
  description: >-
    Reader understands what closed-loop tokens are, how they differ from Coin,
    and can navigate to subtopics on token policies, rules, spending, and action
    requests
  requires:
    - has_frontmatter:
        - title
...
questions:
  - What is Closed-Loop Token in Haneul?
  - What topics does Closed-Loop Token cover?
  - How does closed-loop token work on Haneul?
answer: >-
  Closed-Loop tokens can only be used for a specific service or by authorized
  users.
---
