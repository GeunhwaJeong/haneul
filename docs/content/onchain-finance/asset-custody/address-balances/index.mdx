...
description: >-
  ...
  to Haneul addresses, replacing coin-selection complexity with a single per-address
  accumulator value.
keywords:
  - address balances
  - accumulator
...
questions:
  - What are address balances on Haneul?
  - How does the accumulator model work on Haneul?
...
answer: >-
  Address balances introduce a canonical balance system for fungible assets tied
  to Haneul addresses. The accumulator model maintains one value per (address,
...
| **Gas payment** | Select HANEUL coin objects with `setGasPayment` | Pay directly from HANEUL address balance with no coin selection |
| **Wrapping** | Can be wrapped inside other objects | Cannot be wrapped (not an object) |

## Gas payment from address balances

HANEUL address balances can pay for gas directly, removing the need for coin selection entirely. When you pay gas from an address balance:
...
- No `Coin<HANEUL>` objects need to be provided. The protocol deducts gas from the address balance automatically.
...
For the formal specification of the settlement mechanism, see [SIP-58](https://github.com/haneul-foundation/sips/blob/main/sips/sip-58.md).
