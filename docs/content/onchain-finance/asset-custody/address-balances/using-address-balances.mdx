...
description: >-
  ...
  to Haneul addresses. Learn how to send, withdraw, and query address balances.
keywords:
  - address balances
  - coin objects
  - funds withdrawal
...
questions:
  - What is Using Address Balances in Haneul?
  - How do I send funds?
  - How does using address balances work on Haneul?
answer: >-
  Address balances introduce a canonical balance system for fungible assets tied
  to Haneul addresses. Learn how to send, withdraw, and query address balances.
---

Address balances provide a canonical balance for each coin type tied directly to a Haneul address. Funds sent through `haneul::coin::send_funds` and `haneul::balance::send_funds` merge automatically into a single balance per coin type, with no object management required. For the conceptual model and a comparison with coin objects, see [Address Balances overview](/onchain-finance/asset-custody/address-balances). For the full specification, see [SIP-58: Haneul Address Balances](https://github.com/haneul-foundation/sips/blob/main/sips/sip-58.md).
...
The CLI also provides a dedicated `haneul client send-funds` command that does this without building a PTB:

```bash
# Send 0.005 HANEUL to a recipient's address balance
haneul client send-funds --to <RECIPIENT_ADDRESS> --amount 5000000
...
| `--amount` | Amount to send in GEUNHWA. Required unless you pass `--all-coins`. |
...
For explicit control over address balance withdrawals, use [`tx.withdrawal()`](https://sdk.haneul-labs.com/haneul/transactions/reference#address-balance-withdrawals). The withdrawal must be redeemed through `redeem_funds` to produce a usable `Coin<T>` or `Balance<T>`:
