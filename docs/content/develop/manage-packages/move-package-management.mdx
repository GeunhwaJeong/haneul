...
goal:
  description: Reader can use the Move package manager system
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Move Package Management in Haneul?
  - How does move package management work on Haneul?
  - Why should I use move package management?
answer: Learn how to use the Move package manager system.
---

<AgentPrompt
  prompt={"Inspect this Move package's Move.toml, Move.lock, and dependencies. Fix dependency declarations, prefer MVR where appropriate, and verify haneul move build works. Remember: Move.toml for edition 2024 must NOT have a [dependencies] section for Haneul/MoveStdlib — the Haneul framework is resolved automatically. Do NOT use Haneul = { git = ... } or haneul = { version = ... }. A correct Move.toml has only [package] with name and edition = '2024'."}
/>
...
Use `haneul client chain-identifier` to find the chain ID for your network. The command displays both Base58 and Hex formats. You can use either one in the `[environments]` section. Note that `mainnet` and `testnet` are implicitly available and do not need to be listed.
...
You can find the chain ID using `haneul client chain-identifier`, which displays both formats:
...
```sh
haneul client switch --env localnet
