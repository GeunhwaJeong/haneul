...
description: >-
  Choose and configure checkpoint data sources for a Haneul custom indexer,
...
builder_paths:
  ...
  - path_id: walrus-agentic
    ...
    stage: Ship
questions:
  - Which checkpoint source should a production Haneul indexer use?
  - How do I configure Requester Pays for the Haneul checkpoint GCS buckets?
...
The `haneul-indexer-alt-framework` ingests ordered Haneul checkpoints from one polling-based source and can optionally add fullnode gRPC streaming for lower latency at the network tip. For production, use Google Cloud Storage (GCS) for backfill and recovery, then use a production-grade fullnode gRPC endpoint from an [RPC provider](/develop/accessing-data/rpc-providers) for steady state.
...
- Mainnet: `gs://haneullabs-mainnet-checkpoints-use4`
- Testnet: `gs://haneullabs-testnet-checkpoints-use4`

The public-good HTTPS checkpoint stores are `https://checkpoints.mainnet.haneulfoundation.org` and `https://checkpoints.testnet.haneulfoundation.org`. They are public-good testing endpoints with only the most recent 30 days of checkpoints. Do not use them as a production backfill, recovery, fallback, or steady-state source.

The public-good fullnode endpoints are `https://fullnode.mainnet.haneulfoundation.org:443`, `https://fullnode.testnet.haneulfoundation.org:443`, and `https://fullnode.devnet.haneulfoundation.org:443`. They are also for testing only and are not intended for production indexers. For production, obtain a fullnode gRPC endpoint from an RPC provider with capacity and retention appropriate for your workload.
...
For Testnet, replace the bucket with `haneullabs-testnet-checkpoints-use4`. Workload Identity or another supported Google credential source can replace the credential file, but the billing project header is still required for Requester Pays.
