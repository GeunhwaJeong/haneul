...
The public-good HTTPS checkpoint stores are `https://checkpoints.mainnet.haneul.io` and `https://checkpoints.testnet.haneul.io`. They are public-good testing endpoints with only the most recent 30 days of checkpoints. Do not use them as a production backfill, recovery, fallback, or steady-state source.

The public-good fullnode endpoints are `https://fullnode.mainnet.haneul.io:443`, `https://fullnode.testnet.haneul.io:443`, and `https://fullnode.devnet.haneul.io:443`. They are also for testing only and are not intended for production indexers. For production, obtain a fullnode gRPC endpoint from an RPC provider with capacity and retention appropriate for your workload.
