...
description: >-
  Build a custom indexer using the `haneul-indexer-alt-framework` module. The
  example indexer demonstrates a sequential pipeline that extracts transaction
  digests from Haneul checkpoints and stores them in a local PostgreSQL database,
  the default example storage backend.
keywords:
  - indexer
  - custom indexer
  - indexer framework
...
goal:
  description: >-
    Reader can build a custom indexer using the `haneul-indexer-alt-framework`
    module.
  requires:
    - has_frontmatter:
        - title
...
builder_paths:
  ...
  - path_id: walrus-agentic
    ...
    stage: Ship
questions:
  - How do I build a custom Haneul indexer?
  - What is the haneul-indexer-alt-framework?
  - Where can I find custom indexer examples?
answer: Build a custom indexer using the `haneul-indexer-alt-framework` module.
---

Build a custom indexer using the `haneul-indexer-alt-framework` module.

This guide provides a practical example for building a custom indexer. If you are adding an indexer to a Rust service you already have, see [Integrate a Custom Indexer](/develop/accessing-data/custom-indexer/integrate) instead, which skips the project and database setup. Refer to [Custom Indexing Framework](/develop/accessing-data/custom-indexer/custom-indexers) and [Indexer Pipeline Architecture](/develop/accessing-data/custom-indexer/pipeline-architecture) for a conceptual overview of the indexer framework.
...
- Connects to Haneul checkpoint sources: Uses the durable Requester Pays GCS bucket for production backfill, then fullnode gRPC polling and streaming for steady-state ingestion. The public-good HTTPS checkpoint store is for recent-data testing only.
...
- **GCS buckets (production backfill and recovery):** `gs://haneullabs-testnet-checkpoints-use4` for Testnet or `gs://haneullabs-mainnet-checkpoints-use4` for Mainnet. These buckets have full checkpoint retention and are Requester Pays enabled, so configure GCS credentials and a billing project before using them.
...
- **Public-good fullnode gRPC (testing only):** `https://fullnode.testnet.haneul.io:443`, `https://fullnode.mainnet.haneul.io:443`, and `https://fullnode.devnet.haneul.io:443`. Do not use these endpoints for production ingestion.
- **Public HTTPS checkpoint stores (testing only, most recent 30 days):** `https://checkpoints.testnet.haneul.io` for Testnet or `https://checkpoints.mainnet.haneul.io` for Mainnet. Do not use these public-good endpoints for production ingestion.
...
For testing only, you can instead use the public-good Testnet endpoint `https://fullnode.testnet.haneul.io:443`. The public-good Mainnet, Testnet, and Devnet fullnode endpoints are not intended for production use.
...
The public HTTPS checkpoint stores (`https://checkpoints.testnet.haneul.io` and `https://checkpoints.mainnet.haneul.io`) retain only the most recent 30 days of checkpoints. Use them only for testing. When using one with a fresh database, set `--first-checkpoint` to a checkpoint that is still within the retained range; otherwise, the indexer starts at checkpoint 0 and retries missing checkpoints indefinitely. For production backfills, use `--remote-store-gcs` with `haneullabs-testnet-checkpoints-use4` for Testnet or `haneullabs-mainnet-checkpoints-use4` for Mainnet. See [Use Requester Pays](https://cloud.google.com/storage/docs/using-requester-pays) and [Running a Remote Store](/operators/data-management/remote-store-setup) for more information.
