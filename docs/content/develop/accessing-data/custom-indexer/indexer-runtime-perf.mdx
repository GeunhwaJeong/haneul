...
description: >-
  Learn how to optimize Haneul custom indexer performance through runtime
  configuration, resource monitoring, and debugging tools. Covers ingestion
  settings, database tuning, Tokio console debugging, Prometheus metrics, and
  data pruning strategies.
keywords:
  - haneul indexer performance
  ...
  - haneul blockchain indexing
goal:
  description: >-
    Reader can optimize Haneul custom indexer performance through runtime
    configuration, resource monitoring, and debugging tools.
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is Optimize Indexer Runtime and Performance in Haneul?
  - How do I monitor indexer runtime and performance?
  - How do I troubleshoot indexer runtime and performance?
answer: >-
  Configuration and resource monitoring deliver performant custom indexers. For
  example:.
---
...
let ingestion_config = IngestionConfig {
    ...
    // Minimum boundary (in checkpoints) for an ingestion cohort (default: 25000)
    min_cohort_boundary: 25000,
};
```
...
- `min_cohort_boundary`: Minimum absolute distance from the tip for an ingestion cohort's boundary. Pipelines are grouped into cohorts based on how far behind the tip they are. The closest pipeline seeds a cohort whose boundary is `max(2 * its_distance_from_tip, min_cohort_boundary)`, and all pipelines within that boundary join the same cohort. A higher value prevents nearly caught-up pipelines from being fragmented into many small cohorts, reducing ingestion overhead. A lower value gives finer-grained grouping when pipelines have widely different lag. Default: `25000`.
