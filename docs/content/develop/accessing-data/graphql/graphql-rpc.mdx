...
keywords:
  - graphql
  - graphql headers
  - x-haneul-rpc-version
  - x-haneul-rpc-show-usage
  - variables
  - fragments
  - pagination
  - graphql limits
  ...
  - haneul rpc calls
goal:
  description: Reader can use GraphQL to make Haneul RPC calls
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What is GraphQL for Haneul RPC in Haneul?
  - When to use?
  - How does graphql for haneul rpc work on Haneul?
answer: Use GraphQL to make Haneul RPC calls.
---
...
GraphQL RPC Server and the General-purpose Indexer are generally available. JSON-RPC is deprecated. JSON-RPC is disabled on Haneul Foundation mainnet full nodes the week of July 27, 2026, with full decommission in mid-October 2026. See the [JSON-RPC Migration Guide](/develop/accessing-data/json-rpc-migration) for the complete timeline.
...
- **Rich queries:** A request can contain only a bounded number (default: 5) of queries that require dedicated access to the database (cannot be grouped with other requests).
