...
The `@haneullabs/haneul` TypeScript SDK does not yet expose `SubscribeEvents`. Use the generated proto client directly until the SDK adds a wrapper. The `grpcurl` and Buf CLI examples on this page work today.
...
The `@haneullabs/haneul` TypeScript SDK does not yet expose `ListEvents`. Use the generated proto client directly until the SDK adds a wrapper, or use the Rust SDK, which supports `ListEvents`. The `grpcurl` example in the next tab works today.
