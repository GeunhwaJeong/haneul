...
The Haneul RPC provides a [`queryEvents`](/references/haneul-api#haneulx_queryEvents) method to query onchain packages and return available events. As an example, the following `curl` command queries the DeepBookV3 package on Mainnet for a specific type of event:
