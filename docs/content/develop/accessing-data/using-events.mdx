...
:::info
The `@haneullabs/haneul` TypeScript SDK does not yet expose `ListEvents` or `SubscribeEvents`. Use the generated proto client directly until the SDK adds wrappers, or use `grpcurl` / Buf CLI. The `GetTransaction` example below uses the existing SDK.
:::
