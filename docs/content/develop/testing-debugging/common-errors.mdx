...
description: >-
  Find solutions to common issues with addresses, transactions, and Move
  packages.
keywords:
  - address errors
  - package errors
...
goal:
  description: >-
    Reader can identify common Haneul errors by their message and apply the
    documented fix
  requires:
    - has_frontmatter:
        - title
...
questions:
  - What is Troubleshooting Common Errors in Haneul?
  - How do I address errors?
  - How does troubleshooting common errors work on Haneul?
answer: >-
  Find solutions to common issues with addresses, transactions, and Move
  packages.
---
...
devnet => https://fullnode.devnet.haneulfoundation.org:443 (active)
```
...
Look up the chain ID for your network with `haneul client chain-identifier`. The command displays both Base58 and Hex formats — either one works in the `[environments]` section.
...
In the TypeScript SDK v1, `getObject` returned a `HaneulObjectResponse` with `{ data, error }` fields, so a missing object came back as a response with an `error` field rather than throwing. In v2, `getObject` throws directly when the object does not exist.

The same change applies to multi-get methods like `getObjects`. In v1, these returned `HaneulObjectResponse[]`, a `{ data, error }` wrapper per item. In v2, they return `(Object | Error)[]`, where a missing object appears as an `Error` entry in the array instead of a response wrapper.
...
- [TypeScript SDK migration guide](https://sdk.haneul-labs.com/haneul/migrations/haneul-2.0/json-rpc-migration)
