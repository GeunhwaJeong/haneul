...
description: >-
  Custom upgrade policies are used to upgrade live packages while addressing the
  security risks of single key ownership upgrades.
keywords:
  - upgrade policies
...
goal:
  description: >-
    Reader understands custom upgrade policies are used to upgrade live packages
    while addressing the security risks of single
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - What are custom upgrade policies in Haneul?
  - How do I create a custom upgrade policy for a Move package?
  - What is the difference between UpgradeCap and UpgradeTicket?
answer: >-
  Protecting the ability to upgrade a package onchain using a single key can
  pose a security risk for several reasons:.
---
...
A policy lives in its own Move package so that it can outlive and govern the packages it protects. Add the module to an existing package you control, or create a dedicated one:

```sh
$ haneul move new policy
```

Add a `day_of_week.move` source file to the package's `sources` folder with the following code:
...
### Choosing a package to govern {#creating-testing-package}

With the policy onchain, you need a package for it to govern. Use one of your own packages, or create a throwaway to try the flow end to end:

```sh
$ haneul move new example
```

The rest of this example uses a minimal `example.move` module in `example/sources`:
...
The steps that follow publish this package and then upgrade it to change the value in the `Event` it emits.

### Wiring up the TypeScript SDK {#using-typescript-sdk}

Because you are using a custom upgrade policy, the CLI cannot build the publish and upgrade commands for you. Drive them from a Node.js project instead, either your existing deployment scripts or a small one alongside your package.

The scripts below use ES module syntax, so make sure your `package.json` sets:

```json
{ "type": "module" }
```

Then add the Haneul TypeScript SDK as a dependency:
