...
description: >-
  Understand how Haneul transactions pay for gas, how coinWithBalance selects
...
questions:
  - What is Paying for Transactions in Haneul?
  - What topics does Paying for Transactions cover?
  - How does paying for transactions work on Haneul?
...
answer: >-
  Every Haneul transaction pays for computation and storage. The TypeScript SDK's
...
Every Haneul transaction pays for 2 things: **computational execution** and **long-term object storage**. How you source those funds (from coins, address balances, a sponsor, or at zero cost) depends on what the transaction does and who pays.
...
For HANEUL gas specifically, passing an empty array to `setGasPayment` tells the network to draw gas from the address balance:
...
Haneul offers 4 ways to pay for gas, each suited to different use cases:

| **Strategy** | **Who pays** | **User holds HANEUL?** | **Best for** |
|----------|----------|-----------------|----------|
...
The sender provides one or more `Coin<HANEUL>` objects to cover execution and storage costs. The SDK resolves this automatically when you don't explicitly configure gas. See [Gas Fees](/develop/transaction-payment/gas-in-haneul) for the fee formula and budget rules.

### Address balance gas

Instead of selecting coin objects, the network withdraws gas from the sender's HANEUL address balance. This removes the need for coin lookups at gas-selection time. To use it, set `tx.setGasPayment([])` along with `gasPrice`, `gasBudget`, and `ValidDuring` expiration. When all other transaction inputs are also known at build time (no unresolved `coinWithBalance` intents), the transaction can be built fully offline. See [Paying for gas from address balances](/onchain-finance/asset-custody/address-balances/using-address-balances#paying-for-gas-from-address-balances) for requirements.

### Sponsored transactions

A third party (the sponsor) pays gas on behalf of the user. The user builds the transaction, and the sponsor provides gas payment. Both sign. This is ideal for onboarding new users who don't hold HANEUL. See [Sponsored Transactions](/develop/transaction-payment/sponsor-txn) for the full flow, including data structures, sequence diagrams, and risk considerations.
...
Peer-to-peer transfers of [allowlisted stablecoins](/develop/transaction-payment/gasless-stablecoin-transfers#eligible-stablecoins) (USDC, USDHANEUL, and others) execute at zero gas cost. Eligibility requires:
...
// /develop/transaction-payment/gasless-stablecoin-transfers#eligible-stablecoins
const USDC = '0xUSDC_PACKAGE::usdc::USDC';
const client = new HaneulGrpcClient({ network: 'mainnet', baseUrl: 'https://fullnode.mainnet.haneulfoundation.org:443' });
...
// 2. Swap USDC → HANEUL through a DEX pool
const [haneul] = tx.moveCall({
  target: '0xDEX_PACKAGE::router::swap_exact_input',
  typeArguments: [USDC, '0x2::haneul::HANEUL'],
  arguments: [tx.object('0xPOOL_ID'), usdc, tx.pure.u64(0)], // 0 = min output (set real slippage)
});

// 3. Transfer the swapped HANEUL to the recipient
tx.transferObjects([haneul], recipient);
```
...
// 1. Split off 200 USDC and swap to HANEUL
...
tx.transferObjects([allUsdc, haneul], tx.pure.address(senderAddress));
```
...
2. **Does your app want to pay gas on behalf of the user?**
   ...
   - No: continue.
3. **Does the user have a HANEUL address balance?**
