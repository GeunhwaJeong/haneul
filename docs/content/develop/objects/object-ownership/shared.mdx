...
description: >-
  Anyone can access shared objects on the Haneul network, so care must be taken to
...
goal:
  description: >-
    Reader understands anyone can access shared objects on the Haneul network, so
...
builder_paths:
  - path_id: defi-deepbook
    ...
    stage: Move Contract
questions:
  - What is Shared Objects in Haneul?
...
Anyone can submit a transaction that references a shared object. The Haneul runtime does not enforce access control on shared objects. Your Move code must verify authorization for every privileged operation. Learn more in ([Security Best Practices](/develop/security/best-practices)).
...
You can read shared objects offchain using the same methods as any other Haneul object. Use the object's ID to query its current state:
