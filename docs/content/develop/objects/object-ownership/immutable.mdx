...
description: >-
  Immutable objects cannot be changed, transferred, or deleted. Immutable
  objects cannot have an owner and anyone can use them.
keywords:
  - immutable objects
  - objects immutable
...
goal:
  description: >-
    Reader understands immutable objects cannot be changed, transferred, or
    deleted.
  requires:
    - has_frontmatter:
        - title
...
builder_paths:
  - path_id: agentic-payments
    ...
    step: Immutable objects
    stage: Environment
questions:
  - What is Immutable Objects in Haneul?
  - How do I create immutable objects?
  - When to use immutable objects?
  - How do I test immutable objects?
answer: >-
  Objects on Haneul can either be immutable or mutable. An immutable object cannot
  be mutated, transferred, or deleted. These objects have no owner and are
...
The `store` ability is what lets you freeze an object from outside the module that defines it. For a type without `store`, its defining module can call [`haneul::transfer::freeze_object`](/references/framework/haneul_haneul/transfer#haneul_transfer_freeze_object) instead, which takes `T: key`.
