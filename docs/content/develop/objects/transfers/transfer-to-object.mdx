...
description: >-
  On Haneul, you can transfer objects to objects in the same way you can transfer
...
goal:
  description: >-
    Reader understands on Haneul, you can transfer objects to objects in the same
    way you can transfer objects to addresses
  requires:
    - has_frontmatter:
        - title
        - description
...
questions:
  - How do I transfer to object?
  - What is Transfer to Object in Haneul?
  - Why should I use transfer to object?
answer: >-
  On Haneul, you can transfer objects to objects in the same way you can transfer
  objects to addresses.
---
...
Because there is no difference in the result of the object transfer, the same owned-object query works whether the 32-byte ID is an address or an object ID. Where the legacy `haneulx_getOwnedObjects` JSON-RPC method inferred which one you meant, the GraphQL `ObjectFilter` lets you state it with `ownerKind`.
...
Haneul Foundation disabled JSON-RPC on Mainnet full nodes. If your application still calls `haneulx_getOwnedObjects`, see the [JSON-RPC Migration Guide](/develop/accessing-data/json-rpc-migration).
...
The [`transfer`](/references/framework/haneul_haneul/transfer) module in the Haneul framework defines the core receiving interface in Move:
