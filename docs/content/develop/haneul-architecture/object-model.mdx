...
title: Object Model
description: >-
  Everything on the Haneul blockchain is an object that has metadata, a type of
  ownership, and a referencing scheme.
keywords:
  - object model
  - objects on haneul
  - haneul move package
...
goal:
  description: >-
    Reader understands everything on the Haneul blockchain is an object that has
    metadata, a type of ownership, and a referencing scheme
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  - path_id: agentic-payments
    ...
    step: 'Object model (ownership, shared)'
    stage: Environment
    eval: covered
  - path_id: walrus-agentic
    path_name: Walrus Agentic Data Storage
    step: Object model concepts
    stage: Environment
builder_paths_source:
  dashboard: 'https://docs-analytics-dashboard.vercel.app/evals'
  snapshot: '2026-07-10'
questions:
  - What is Object Model in Haneul?
  - How does object model work on Haneul?
  - Why should I use object model?
answer: >-
  Everything on the Haneul blockchain is an object that has metadata, a type of
  ownership, and a referencing scheme.
---

An object is a fundamental unit of storage on the network. Every resource, asset, or piece of data onchain is an object. Many other blockchains structure storage around accounts containing key-value stores. Haneul's storage is structured around objects addressable by unique IDs onchain.
...
- **Haneul object:** Every resource, asset, or piece of data. Haneul objects are what transactions interact with, and are the building blocks for all onchain state.

- **Haneul Move object:** An object defined using the Move language. To create a Move object, you must use `haneul::object::new` to create a struct. This struct must have the `key` ability and include a field `id: UID` as the first defined field. The struct can also contain primitive types, such as integers and addresses, other objects, and non-object structs. This struct can be stored onchain as a Haneul object.

- **Haneul Move package:** A smart contract that can manipulate other objects. Each Move package is a set of Haneul Move bytecode modules. The modules within the package each have a name that's unique within the containing package. The combination of the package's onchain ID and the name of a module uniquely identifies the module. When you publish smart contracts to Haneul, a package is the unit of publishing. After you publish a package object, it is immutable and can never be changed or removed. A package object can depend on other package objects that were previously published to Haneul.
...
Live objects are available as input for a transaction. The global state maintained by Haneul consists of the totality of such objects. The live objects for a particular Haneul address `A` are all objects owned by `A`, along with all shared and immutable objects in the system.
...
Haneul has some limits on transactions and data used in transactions, such as a [maximum size and number of objects used](https://move-book.com/guides/building-against-limits).

The `ProtocolConfig` struct in the [`haneul-protocol-config` crate](https://github.com/GeunhwaJeong/haneul/blob/main/crates/haneul-protocol-config/src/lib.rs) itemizes these limits:
