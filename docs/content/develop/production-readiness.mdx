...
description: >-
  Prepare a Haneul application for Mainnet deployment with checklists covering
...
goal:
  description: >-
    Reader can evaluate their Haneul application against a structured production
...
questions:
  - How do I prepare a Haneul application for Mainnet?
  - What is the production readiness checklist for Haneul?
  - What should I check before deploying a Move package to Mainnet?
  - How do I secure my Haneul deployment for production?
  - What key management practices should I follow for Mainnet?
answer: >-
  Production readiness for a Haneul application involves verifying smart contract
...
Production readiness is the process of verifying that a Haneul application is secure, correctly configured, and operationally prepared before deploying to [Mainnet](/develop/haneul-architecture/networks). Moving from Testnet to Mainnet introduces real financial risk, so each area below requires deliberate verification rather than assumption.
...
- [ ] **Gas budget estimation.** Run critical transaction paths with dry run to measure actual gas costs. Set gas budgets with sufficient margin for variable costs (shared object contention, dynamic field depth). Learn more about [gas in Haneul](/develop/transaction-payment/gas-in-haneul).
- [ ] **Sponsored transactions.** If using [gas sponsorship](/develop/transaction-payment/sponsor-txn), verify that the gas station has sufficient HANEUL balance and rate limiting appropriate for your expected transaction volume.
