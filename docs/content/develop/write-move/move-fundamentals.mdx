...
questions:
  - What is Move Syntax Fundamentals in Haneul?
  - How does move syntax fundamentals work on Haneul?
...
Move is a resource-oriented programming language designed for safe smart contract development. Haneul uses Move with extensions for the [object model](/develop/objects), [programmable transaction blocks](/develop/transactions/ptbs/prog-txn-blocks), and the Haneul-specific framework modules. This page covers the core language syntax. For the full language reference, see [The Move Book](https://move-book.com/).
...
| `address` | A 32-byte address value representing an account or object location on Haneul |

See [The Move Book: Primitive Types](https://move-book.com/move-basics/primitive-types.html).

## Structs

Structs define custom data types with named fields. On Haneul, structs with the `key` ability represent [objects](/develop/objects) and must have `id: UID` as their first field. See [The Move Book: Structs](https://move-book.com/move-basics/struct.html).
...
| **Ability** | **Grants** |
|---|---|
| `key` | The type can be stored as a Haneul object (requires `id: UID` as first field) |
...
Move supports inline unit tests using the `#[test]` attribute. Test functions run during `haneul move test` and do not deploy onchain. Use `#[test_only]` for helper modules that exist only for testing. See [The Move Book: Testing](https://move-book.com/move-basics/testing.html).
