...
description: >-
  The life of a transaction on the Haneul network has some differences compared to
  those from other blockchains.
keywords:
...
goal:
  description: >-
    Reader understands each stage of a transaction's lifecycle on Haneul, from
    creation through consensus, finality, and checkpoints
  requires:
    - has_frontmatter:
        - title
        - description
...
builder_paths:
  ...
  - path_id: walrus-agentic
    ...
    stage: Ship
questions:
  - What is Life of a Transaction in Haneul?
  - How does life of a transaction work on Haneul?
  - Why should I use life of a transaction?
answer: >-
  At a high level, the following diagram outlines the life cycle of a
  transaction on the Haneul blockchain. All transactions are sequenced by Haneul's
  [Mysticeti DAG consensus](/develop/haneul-architecture/consensus).
---
...
- If the lookup returns transaction details, Haneul has finalized and executed the transaction.

- If the lookup finds nothing, either Haneul never finalized the transaction or this full node does not yet know about it. In this case, the safer option is to resubmit the transaction; as noted above, Haneul transactions are idempotent.
