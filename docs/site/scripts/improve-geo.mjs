...
function getArchetype(relPath) {
  if (relPath.startsWith('snippets/') || relPath.includes('haneul-graphql/beta/reference/')) return 'skip';
...
function improveQuestions(questions, title, description, headings, archetype) {
  ...
  function addQ(q) {
    ...
    q = q.replace(/SDK SDK/g, 'SDK')
         ...
         .replace(/in Haneul in Haneul/g, 'in Haneul')
         .replace(/on Haneul on Haneul/g, 'on Haneul')
         .replace(/install haneul\b(?! CLI)/gi, 'install the Haneul CLI')
         .replace(/\s{2,}/g, ' ')
         .trim();
    if (!q.endsWith('?')) q += '?';
  ...
  switch (archetype) {
    case 'onboarding':
      ...
      break;
    case 'example':
      addQ(`How do I build a ${topicAction.replace(/\s+pattern$/i, '')} on Haneul?`);
      break;
    case 'migration': {
      const platform = topicName.replace(/->|→/g, ' ').replace(/\s+(to\s+)?Haneul$/i, '').trim();
      addQ(`How is Haneul different from ${platform}?`);
      addQ(`How do I migrate from ${platform} to Haneul?`);
      break;
    }
    case 'operator':
      addQ(`How do I ${topicAction}?`);
    ...
    default:
      if (/^(build|create|set up|configure|install|deploy|write|test|debug|connect|query|emit|use|run|monitor|sign|send|mint|transfer|upgrade|publish)/i.test(topicAction)) {
        ...
        addQ(`What is ${topicName} in Haneul?`);
      }
  }
