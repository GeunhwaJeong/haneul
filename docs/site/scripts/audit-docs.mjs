  ...
  for (const req of goal.requires) {
    ...
    if (req.pattern !== undefined && req.min !== undefined) {
      ...
      const junkPatterns = [
        ...
        /How do I (the|a|an|haneul|set up haneul|operator|data) /i,     // bad starts
        ...
        /How do I build a [a-z]+ on Haneul\?$/i,                      // "How do I build a plinko on Haneul?" (single word)
        /How do I build a [a-z]+ (testing|apps) on Haneul/i,         // "How do I build a scenario testing on Haneul?"
        /How do I add your /i,                                      // "How do I add your sdk?"
        /^What you learn\??$/i,                                     // heading fragment "What you learn?"
        /^What is [A-Z][a-z]+ a [A-Z]/i,                          // "What is Build a Custom Indexer in Haneul?"
        /(in|on) Haneul (in|on) Haneul/i,                                // "What is X on Haneul in Haneul?"
        /-> Haneul in Haneul/i,                                          // "What is Ethereum -> Haneul in Haneul?"
        /\bon haneul\b/,                                               // lowercase "on haneul" is always a template artifact (proper noun is "Haneul")
      ];
      let junkCount = 0;
      if (has) {
