...
function generateQuestions(title, description, headings, relPath) {
  ...
  switch (archetype) {
    case 'onboarding':
      questions.push(`How do I ${actionForm} on Haneul?`);
      break;
    case 'example':
      questions.push(`How do I build a ${topicLower} on Haneul?`);
      // Avoid "pattern pattern"
      const patternName = topicLower.replace(/\s+pattern$/i, '');
      questions.push(`What is the ${patternName} pattern in Haneul?`);
      break;
    case 'migration': {
      const platform = topicName.replace(/->|→/g, ' ').replace(/\s+to\s+Haneul/i, '').replace(/\s+Haneul$/i, '').trim();
      questions.push(`How does Haneul compare to ${platform}?`);
      questions.push(`How do I migrate from ${platform} to Haneul?`);
      break;
    }
    case 'operator':
      questions.push(`How do I ${actionForm} for Haneul?`);
      break;
    case 'sdk': {
    ...
    case 'reference':
      questions.push(`What is ${topicName} in Haneul?`);
      break;
    case 'index':
      questions.push(`What is ${topicName} in Haneul?`);
      break;
    default: // guide
      ...
      if (/^(build|create|set up|configure|install|deploy|write|test|debug|connect|query|emit|use|optimiz|integrat|run|monitor|sign|send)/i.test(actionForm)) {
        ...
        questions.push(`What is ${topicName} in Haneul?`);
        questions.push(`How does ${topicLower} work on Haneul?`);
      }
  }
...
function main() {
  ...
  for (const filePath of files) {
    ...
    if (relPath.includes('haneul-graphql/beta/reference/')) { skipped++; continue; }
