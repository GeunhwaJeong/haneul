...
function generateDescription(relPath, data, body) {
  ...
  if (relPath.startsWith('getting-started/onboarding/')) {
    const specifics = {
      'haneul-install.mdx': 'Reader can install the Haneul CLI, verify it works, and understand available toolchain components',
      'configure-haneul-client.mdx': 'Reader can configure the Haneul client for Testnet, Devnet, or a custom network and verify connectivity',
      'get-address.mdx': 'Reader can create a Haneul address, understand recovery phrases, and manage multiple addresses',
      'get-coins.mdx': 'Reader can request HANEUL tokens from the faucet and confirm they arrived in their wallet',
      ...
      'local-network.mdx': 'Reader can start a local Haneul network, fund addresses, and test against it instead of Testnet',
      'install-source.mdx': 'Reader can build the Haneul CLI from source when prebuilt binaries are not suitable',
      'install-binaries.mdx': 'Reader can install prebuilt Haneul binaries for their platform',
    ...
    return `Reader can ${ctLower} on Haneul and confirm it worked`;
  }
  ...
  // ── Migration guides ──
  if (relPath.startsWith('getting-started/haneul-for-')) {
    const platform = title.includes('->') ? title.split('->')[0].trim()
      : title.includes('→') ? title.split('→')[0].trim()
      : title.replace(/^Haneul for /i, '').trim();
    return `${platform} developer can map their existing mental model to Haneul's object-centric equivalents and start building`;
  }
  ...
  if (relPath === 'getting-started/tooling.mdx') {
    return 'Reader can find and choose the right developer tool for writing, testing, deploying, or auditing Move on Haneul';
  }

  // ── CLI reference ──
  if (relPath.startsWith('references/cli/')) {
    if (relPath.endsWith('cheatsheet.mdx')) {
      return 'Reader can quickly find the right CLI command for common Haneul operations';
    }
  ...
  if (relPath.startsWith('references/')) {
    if (relPath.includes('contribute/')) {
      return `Contributor understands the ${ctLower} requirements for the Haneul documentation`;
    }
    ...
    if (title.toLowerCase().includes('glossary')) {
      return 'Reader can look up unfamiliar Haneul terms and concepts by name';
    }
    if (title.toLowerCase().includes('release notes')) {
      return 'Reader can review what changed in each Haneul release';
    }
    if (title.toLowerCase().includes('framework')) {
      return 'Reader can browse the Haneul framework module reference to find function signatures and type definitions';
    }
    if (desc) {
  ...
  // ── Haneul stack ──
  if (relPath.startsWith('haneul-stack/')) {
    if (desc) {
      const outcome = descToOutcome(desc);
  ...
  if (relPath.includes('testing-debugging/')) {
    if (title.toLowerCase().includes('error') || title.toLowerCase().includes('troubleshoot')) {
      return 'Reader can identify common Haneul errors by their message and apply the documented fix';
    }
    if (desc) {
      const cleaned = desc.replace(/\.$/, '');
      return `Reader can ${cleaned.charAt(0).toLowerCase()}${cleaned.slice(1)}`;
