...
function getArchetype(relPath, data, body) {
  ...
  if (relPath.includes('haneul-graphql/beta/reference/')) return 'skip';
  ...
  if (relPath.startsWith('getting-started/haneul-for-')) return 'migration';
  ...
  // Haneul stack
  if (relPath.startsWith('haneul-stack/')) return 'guide';
...
function generateOnboardingGoal(title, body, headings, codeBlocks, h2s, relPath) {
  ...
  const onboardingOrder = [
    'haneul-install', 'configure-haneul-client', 'get-address',
  ...
  if (hasPattern(body, 'install|haneulup')) {
    requires.push({ pattern: 'haneul --version|haneul -V', min: 1, label: 'Shows how to verify installation' });
  }
  ...
  if (hasPattern(body, 'haneul move build|haneul client publish')) {
...
function generateMigrationGoal(title, body, headings) {
  ...
  return {
    description: `Reader can map familiar ${platform || 'platform'} concepts to their Haneul equivalents`,
    requires,
  };
}
...
function generateCatalogGoal(title, wc) {
  return {
    description: `Reader can find the right tool for their Haneul development task`,
    requires: [
...
function generateCliReferenceGoal(title, body) {
  ...
  requires.push({ pattern: 'haneul ', min: 2, label: 'Shows CLI command usage' });
