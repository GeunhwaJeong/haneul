...
use haneullabs_common::sync::notify_read::OwnedRegistration;
...
/// includes it in a block, or with an error on eviction or epoch end.
pub type PositionReceiver = oneshot::Receiver<HaneulResult<Vec<ConsensusPosition>>>;
...
enum EntryAck {
    ...
    User(oneshot::Sender<HaneulResult<Vec<ConsensusPosition>>>),
...
impl PendingAck {
    ...
    fn resolve_error(mut self, error: HaneulError) {
        match self.ack.take().expect("ack must be pending") {
            EntryAck::User(sender) => {
...
impl AdmissionQueueEntry for PoolEntry {
    ...
    fn notify_evicted(self, min_gas_price: u64) {
        ...
        self.ack.resolve_error(
            HaneulErrorKind::TransactionRejectedDueToOutbiddingDuringCongestion { min_gas_price }
                .into(),
        );
    }
...
impl ConsensusTransactionPool {
    ...
    ) -> HaneulResult<(PositionReceiver, bool)> {
    ...
    ) -> HaneulResult<oneshot::Receiver<BlockInclusion>> {
        let (sender, receiver) = oneshot::channel();
    ...
    ) -> HaneulResult<Option<ProcessedMethod>> {
    ...
    fn check_user_lane_open_locked<'a>(
        ...
        caller_epoch: EpochId,
        inner: &'a mut Inner,
    ) -> HaneulResult<&'a mut PriorityAdmissionQueue<PoolEntry>> {
        let pool = self.check_epoch_and_open_locked(caller_epoch, inner)?;
        match &mut pool.user {
            UserLane::Open(user) => Ok(user),
            UserLane::Closed => Err(HaneulErrorKind::ValidatorHaltedAtEpochEnd.into()),
        }
    }
    ...
    fn serialize_and_validate(
        &self,
        transactions: &[ConsensusTransaction],
    ) -> HaneulResult<(Vec<Transaction>, usize)> {
    ...
    fn resolve_flushed_user_entries(&self, entries: Vec<PoolEntry>) {
        for entry in entries {
            ...
            entry
                .ack
                .resolve_error(HaneulErrorKind::ValidatorHaltedAtEpochEnd.into());
        }
    }
...
fn consensus_client_error(error: ClientError) -> HaneulError {
    HaneulErrorKind::FailedToSubmitToConsensus(error.to_string()).into()
}
...
impl TransactionPoolContext {
    ...
    pub async fn wait_for_pool(
        &self,
        caller_epoch: EpochId,
    ) -> HaneulResult<Arc<ConsensusTransactionPool>> {
...
mod tests {
    ...
    use haneul_types::base_types::{AuthorityName, random_object_ref};
    ...
    fn user_transaction() -> ConsensusTransaction {
        ...
        let transaction = crate::test_utils::make_transfer_haneul_transaction(
            random_object_ref(),
            sender,
            Some(1),
            sender,
