...
use haneul_config::transaction_deny_config::{
...
use haneul_types::committee::{Committee, StakeUnit, TOTAL_VOTING_POWER};
...
use haneul_types::transaction_deny_rules::{DenyElement, DenyElementKind, TransactionDenyRules};
...
impl TransactionDenyConfigManager {
    pub fn new(
        ...
        sync_config.validate().map_err(HaneulError::from)?;
        ...
        // Startup reconciliation (in haneul-node) either re-broadcasts the current local
    ...
    pub fn update_for_committee(&self, committee: Arc<Committee>) -> HaneulResult<()> {
        let (evaluation, active_proposals) = {
            let mut peer_configs = self.peer_configs.lock();
    ...
    pub fn allocate_next_broadcast_generation(&self) -> HaneulResult<u64> {
    ...
    ) -> HaneulResult<SharedTransactionDenyConfig> {
        if let Some(rules) = &rules {
            rules.check_share_limits().map_err(HaneulError::from)?;
        }
    ...
    pub fn submit_broadcast(
        ...
        // verification-time path in HaneulTxValidator never sees this message, and waiting
...
impl TransactionDenyConfigMetrics {
    pub fn new(registry: &Registry) -> Self {
        Self {
            ...
            // `haneul_config::node_config_metrics`, so existing dashboards keep working.
...
mod tests {
    ...
    use haneul_types::base_types::{ObjectID, dbg_addr};

    fn fake_name(byte: u8) -> AuthorityName {
        AuthorityName::new([byte; haneul_types::crypto::AuthorityPublicKey::LENGTH])
    }
