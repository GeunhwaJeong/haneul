...
use haneul_types::base_types::ConsensusObjectVersion;
...
use haneul_types::{
    HANEUL_RANDOMNESS_STATE_OBJECT_ID, IMPLICITLY_READ_SYSTEM_OBJECTS, base_types::SequenceNumber,
    error::HaneulResult,
};
...
impl AssignedVersions {
    ...
    pub fn new_for_testing(
        ...
        Self::new(
            ...
            SystemObjectVersions::new(accumulator_version.map(|v| ConsensusObjectVersion {
                initial_shared_version: haneul_types::object::OBJECT_START_VERSION,
                version: v,
            })),
        )
    }
    ...
    pub fn accumulator_version(&self) -> Option<SequenceNumber> {
        self.system_object_versions
            .get(&HANEUL_ACCUMULATOR_ROOT_OBJECT_ID)
            .map(|v| v.version)
    }
    ...
    pub fn assign_versions_from_effects(
        ...
        for (cert, effects, accumulator_version) in certs_and_effects {
            ...
            if let (Some(effects_version), Some(sequenced_version)) = (
                accessed_versions.get(&HANEUL_ACCUMULATOR_ROOT_OBJECT_ID),
                accumulator_version,
