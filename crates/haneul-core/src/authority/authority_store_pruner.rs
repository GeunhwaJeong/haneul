...
pub use haneul_rpc_store::RetractionCursors;
...
impl AuthorityStorePruner {
    ...
    /// ([`haneul_rpc_store::embedded_prunable_checkpoint`]). The indexer's
    ...
    fn rpc_store_max_eligible_checkpoint(
        ...
        let indexed = haneul_rpc_store::embedded_prunable_checkpoint(rpc_store.db())?;
...
mod tests {
    ...
    use haneul_rpc_store::RetractionCursors;
    ...
    fn rpc_store_gate_bounds_eligible_checkpoints() {
        ...
        let store = haneul_rpc_store::Store::new(db.clone(), Arc::new(schema));
