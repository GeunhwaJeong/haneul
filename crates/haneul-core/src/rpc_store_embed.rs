...
//! [`RpcConfig::enable_indexing`](haneul_config::RpcConfig::enable_indexing), it
//! serves the rpc-api index surface from an embedded [`haneul_rpc_store`]
...
use haneul_rpc_store::schema::event_bitmap;
use haneul_rpc_store::schema::transaction_bitmap;
...
impl EmbeddedRpcStore {
    ...
    pub async fn bootstrap(
        ...
        let state = StoreState {
            ...
            restore_in_progress: haneul_rpc_store::restore_in_progress(&db)?,
            history_seed_pending: haneul_rpc_store::history_seed_pending(&db)?,
        };
    ...
    /// ([`haneullabs_common::fatal!`]): it will not heal on its own, and a
...
#[cfg(test)]
mod tests {
    use haneul_rpc_store::schema::pruning_watermark;
