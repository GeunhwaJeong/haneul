...
| `haneul-replay-2` | Reconstructed from expected effects (`SystemObjectVersions::from_effects`). **Caveat:** unsettled in-commit withdrawals are *not* reconstructed in isolated replay (`unsettled = 0`), which can diverge from the original execution — see the TODO in `crates/haneul-replay-2/src/execution.rs`. Mainnet enablement is blocked on this. |
...
- The native's abort surfaces as an ordinary framework abort,
  ...
  `haneul_types::funds_accumulator::is_object_funds_insufficient_abort`.
