...
use crate::authority::authority_store::{
    HaneulLockResult, LockDetailsDeprecated, ObjectLockStatus,
};
...
impl WritebackCache {
    ...
    pub(crate) fn load_implicitly_read_system_object(
        ...
        assert!(
            haneul_types::IMPLICITLY_READ_SYSTEM_OBJECTS.contains(object_id),
...
impl ObjectCacheRead for WritebackCache {
    ...
    fn get_lock(
        ...
        let Some(obj) = self.get_object_impl("lock", &obj_ref.0) else {
            return Err(HaneulError::from(UserInputError::ObjectNotFound {
                object_id: obj_ref.0,
