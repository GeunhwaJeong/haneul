...
use haneul_types::base_types::{ConsensusObjectVersion, HaneulAddress, ObjectID, SequenceNumber};
use haneul_types::object::{Object, Owner};
use haneul_types::storage::{BackingStore, InputKey, TrackingBackingStore};
use std::path::Path;
...
async fn test_load_implicitly_read_system_object() {
    ...
    let object_id = haneul_types::HANEUL_ACCUMULATOR_ROOT_OBJECT_ID;
    ...
    let blocked = tokio::task::spawn_blocking({
        ...
        move || {
            let _permit_guard =
                haneullabs_common::sync::execution_permit::set_execution_permit(Box::new(permit));
