...
impl<C: CheckpointServiceNotify + Send + Sync> ConsensusHandler<C> {
    ...
    /// block verification in `HaneulTxValidator` (re-application here is dropped as a stale
...
mod tests {
    ...
    // HANEUL_ENABLE_DEBUG_ASSERTIONS); in a plain release build it logs and continues, so the
        ...
        let make_update = |authority, generation| {
            ConsensusTransaction::new_update_transaction_deny_config(
                SharedTransactionDenyConfig::V1(
                    haneul_types::messages_consensus::SharedTransactionDenyConfigV1 {
                        authority,
                        generation,
