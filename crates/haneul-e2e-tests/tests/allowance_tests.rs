...
fn balance_haneul_type() -> haneul_types::TypeTag {
    "0x2::balance::Balance<0x2::haneul::HANEUL>"
        .parse()
        .unwrap()
}

fn rate_limit_type() -> haneul_types::TypeTag {
...
async fn test_allowance_issue_and_spend() {
    ...
    // framework, which includes `haneul::allowance`, and enables the flag.
...
async fn test_allowance_spender_rotated_mid_flight() {
    ...
    let pkg = test_env
        ...
        .await;
    let app_type: haneul_types::TypeTag = format!("{pkg}::allowance_app::APP").parse().unwrap();
    ...
    let proposal = builder.programmable_move_call(
        ...
        vec![balance_haneul_type(), app_type.clone()],
        args,
    );
    builder.programmable_move_call(
        pkg,
