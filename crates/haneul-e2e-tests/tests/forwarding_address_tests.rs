...
use haneul_types::HANEUL_FORWARDING_ADDRESS_REGISTRY_OBJECT_ID;
...
/// protocol version 122, since only freshly-created networks get it at genesis. Genesis at
/// protocol version 121 uses the frozen v121 framework snapshot, which predates the
/// `forwarding_address` module.
#[sim_test]
async fn test_create_forwarding_address_registry_object_at_upgrade() {
    let _guard = haneul_protocol_config::ProtocolConfig::apply_overrides_for_testing(
        |version, mut config| {
            // The flag is devnet-only; force it on so the test also runs under the mainnet
            // chain override. Only from version 122: at 121 the frozen framework snapshot
            // predates the forwarding_address module, so genesis must not call it.
            if version.as_u64() >= 122 {
    ...
    let test_cluster = TestClusterBuilder::new()
        .with_protocol_version(121.into())
        .with_epoch_duration_ms(10000)
        .with_supported_protocol_versions(SupportedProtocolVersions::new_for_testing(121, 122))
        .build()
        .await;
    ...
    test_cluster.wait_for_protocol_version(122.into()).await;
