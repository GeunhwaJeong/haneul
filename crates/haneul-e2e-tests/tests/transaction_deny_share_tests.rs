...
use haneul_types::base_types::{AuthorityName, FullObjectRef, ObjectID};
use haneul_types::messages_consensus::{
    ConsensusTransaction, SharedTransactionDenyConfig, SharedTransactionDenyConfigV1,
};
use haneul_types::transaction_deny_rules::TransactionDenyRules;
use test_cluster::{TestCluster, TestClusterBuilder};

const PROPAGATION_TIMEOUT: Duration = Duration::from_secs(60);

fn enable_protocol_flags() -> haneul_protocol_config::OverrideGuard {
    haneul_protocol_config::ProtocolConfig::apply_overrides_for_testing(|_, mut c| {
...
async fn broadcast_via_consensus(
    sender_handle: &HaneulNodeHandle,
...
fn handle_for(handles: &[HaneulNodeHandle], name: AuthorityName) -> HaneulNodeHandle {
    handles
        .iter()
...
fn effective_denies_obj(handle: &HaneulNodeHandle, id: ObjectID) -> bool {
    handle.with(|node| {
        node.state()
            .transaction_deny_config_manager()
            .effective_config()
...
fn effective_user_transaction_disabled(handle: &HaneulNodeHandle) -> bool {
    handle.with(|node| {
        node.state()
            .transaction_deny_config_manager()
            .effective_config()
...
fn ruleset_is_active(handle: &HaneulNodeHandle, name: &str) -> bool {
    handle.with(|node| {
        node.state()
            .transaction_deny_config_manager()
            .evaluate_status()
