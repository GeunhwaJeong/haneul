...
use haneul_config::node::ConsensusTransactionPoolConfig;
...
async fn test_transaction_pool_eviction_and_rejection() {
    use haneul_macros::{clear_fail_point, register_fail_point_if};
    ...
    let cluster = TestClusterBuilder::new()
        ...
        .await;
    let validator = cluster.all_validator_handles().into_iter().next().unwrap();
    let target_node = validator.with(|_| haneul_simulator::current_simnode_id());
    let disabled = take_disabled.clone();
    validator.with(|_| {
        register_fail_point_if("consensus_transaction_pool_disable_take", move || {
            haneul_simulator::current_simnode_id() == target_node && disabled.load(Ordering::SeqCst)
        });
    });
...
async fn test_transaction_pool_epoch_store_swap_waits_for_new_pool() {
    use haneul_macros::{clear_fail_point, register_fail_point_async};
    use std::num::NonZeroUsize;
    use std::sync::Arc;
    use std::sync::atomic::{AtomicBool, Ordering};
    use std::time::Duration;
