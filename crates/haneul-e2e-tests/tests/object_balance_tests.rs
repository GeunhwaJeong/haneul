...
fn assert_object_funds_insufficient(
    status: &haneul_types::execution_status::ExecutionStatus,
    context: &str,
) {
    assert!(
        matches!(
...
async fn simulate_object_funds_and_check_root(
    test_env: &TestEnv,
    tx: &haneul_types::transaction::TransactionData,
) -> TransactionEffects {
    ...
    let root_id = HANEUL_ACCUMULATOR_ROOT_OBJECT_ID.to_string();
    assert!(
...
async fn test_object_funds_unsettled_withdraws_same_commit() {
    ...
    let withdraw_tx = |test_env: &TestEnv, sender: HaneulAddress, amount: u64| {
        test_env
            .tx_builder(sender)
            .transfer_haneul_to_address_balance(
    ...
    let fullnode = test_env.cluster.spawn_new_fullnode().await.haneul_node;
    fullnode
        .state()
