...
async fn execute_pinned_proof_tx(
    ...
    let eph_kp = HaneulKeyPair::Ed25519(Ed25519KeyPair::generate(&mut StdRng::from_seed([0; 32])));
    let zklogin_addr = HaneulAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap());
...
async fn test_zklogin_circuit_mode_pinned_proofs() {
    ...
    haneul_node::set_jwk_injector(Arc::new(move |_, _| Ok(injected.clone())));
...
async fn zklogin_v1_to_v2_migration_scenario_test() {
    ...
    let verify = |proof_json: &str, zklogin_circuit_mode: u64| -> (HaneulResult, Logs) {
        let inputs = ZkLoginInputs::from_json(proof_json, PINNED_PROOF_ADDRESS_SEED).unwrap();

        let mut config = ProtocolConfig::get_for_max_version_UNSAFE();
        config.set_zklogin_circuit_mode_for_testing(zklogin_circuit_mode);

        let author = HaneulAddress::from(&PublicKey::from_zklogin_inputs(&inputs).unwrap());
