...
async fn test_deposits() {
    ...
    test_env.cluster.fullnode_handle.haneul_node.with(|node| {
        ...
        let accumulator_object =
            AccumulatorValue::load_object(runtime_object_resolver, None, recipient, &haneul_coin_type)
    ...
    assert!(
        matches!(
            ...
            haneul_types::execution_status::ExecutionStatus::Failure(failure)
                if haneul_types::funds_accumulator::is_object_funds_insufficient_abort(&failure.error)
        ),
...
/// same `(sender, Balance<HANEUL>)` key. With in-execution object-funds checking, the native withdraw
...
/// Regression shape for a single unbacked object-sourced `u64::MAX` HANEUL withdrawal. The
/// in-execution object-funds check rejects it at withdrawal time, before accumulator
/// representability or HANEUL-conservation checks need to reason about the oversized event.
#[sim_test]
...
async fn accumulator_conservation_overflow_single_withdrawal(check_in_execution: bool) {
    ...
    // A single u64::MAX HANEUL withdrawal deposited to the sender, paid with a normal gas coin.
    let (sender, gas) = test_env.get_sender_and_gas(0);
    let mut builder = ProgrammableTransactionBuilder::new();
    builder.programmable_move_call(
        pkg,
...
/// withdrawal to the sender's HANEUL address balance. The in-execution object-funds check aborts before
...
/// Regression shape for gas-coin overflow via `Argument::GasCoin`: two object-sourced HANEUL
/// withdrawals were redeemed to `Coin<HANEUL>` and merged into the gas coin before a net gas refund.
