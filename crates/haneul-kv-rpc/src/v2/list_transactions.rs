...
use haneul_inverted_index::ScanStop;
...
use haneul_rpc::proto::haneul::rpc::v2::QueryEnd;
...
use haneul_rpc_api::ledger_history::query_options::RangeExhaustion;
use haneul_rpc_api::ledger_history::query_options::ResolvedCheckpointRange;
use haneul_rpc_api::ledger_history::query_options::ResolvedScan;
use haneul_rpc_api::ledger_history::query_options::validate_checkpoint_bounds;
use haneul_rpc_api::ledger_history::watermark::ScanTerminal;
use haneul_rpc_api::ledger_history::watermark::advance_covered_bound_before_checkpoint;
use haneul_rpc_api::ledger_history::watermark::boundary_watermark;
use haneul_rpc_api::ledger_history::watermark::item_watermark;
use haneul_rpc_api::ledger_history::watermark::scan_frontier_cursor_cp;
...
mod tests {
    ...
    use haneul_kvstore::testing::insert_checkpoint_rows;
    ...
    async fn digest_resolution_records_all_metrics_without_hydration() {
        ...
        let mut builder = haneul_types::test_checkpoint_data_builder::TestCheckpointBuilder::new(0);
        ...
        assert!(calls.iter().all(|call| {
            call.table != haneul_kvstore::tables::transactions::NAME
                && call.table != haneul_kvstore::tables::objects::NAME
        }));
        server.abort();
    }
        ...
        for (mask, identity_only) in [
            ...
            if identity_only {
                let mut actual_object_rows = mock
                    ...
                    .await
                    .into_iter()
                    .filter(|call| call.table == haneul_kvstore::tables::objects::NAME)
                ...
                let mut expected_object_rows = (0..checkpoint.transactions.len())
                    ...
                    .collect::<BTreeSet<_>>()
                    .into_iter()
                    .map(|key| haneul_kvstore::tables::objects::encode_key(&key))
                    .collect::<Vec<_>>();
        ...
        for (
            ...
            if !direction.is_ascending() {
                proto_options.ordering =
                    Some(haneul_rpc::proto::haneul::rpc::v2::Ordering::Descending as i32);
            }
            let options =
