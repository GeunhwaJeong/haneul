...
mod test {
    ...
    ) -> std::collections::HashMap<haneul_simulator::task::NodeId, AuthorityName> {
        test_cluster
            .swarm
            .validator_nodes()
    ...
    fn register_fork_kill_failpoints(
        ...
        resolve_authority: impl Fn(haneul_simulator::task::NodeId) -> Option<AuthorityName>
        + Clone
        + Send
        + Sync
        + 'static,
        ...
        register_fail_point_arg("kill_checkpoint_fork_node", {
            ...
            move || {
                let current = resolve_authority(haneul_simulator::current_simnode_id())?;
        ...
        register_fail_point_if("kill_transaction_fork_node", {
            ...
            let resolve_authority = resolve_authority.clone();
            move || {
                resolve_authority(haneul_simulator::current_simnode_id())
            ...
            Some({
                ...
                move |_cluster: Arc<TestCluster>| async move {
                    ...
                    register_fail_point_arg("simulate_fork_during_execution", {
                        ...
                        let node_to_authority_map = node_to_authority_map.clone();
                        move || {
                            let current = node_to_authority_map
    ...
    async fn test_auto_fork_recovery_transaction_fork() {
        ...
        let tx_data = test_cluster
            ...
            .await
            .transfer_haneul(Some(1), test_cluster.get_address_1())
            .build();
        ...
        let known_other_ids: std::collections::HashSet<haneul_simulator::task::NodeId> =
            test_cluster
                .swarm
                .all_nodes()
                .filter_map(|node| {
        ...
        register_fail_point_arg("simulate_fork_during_execution", {
            ...
            move || {
                if known_other_ids.contains(&haneul_simulator::current_simnode_id()) {
                    None
                } else {
                    Some((
                        forked_validators.clone(),
