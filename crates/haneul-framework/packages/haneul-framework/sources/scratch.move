...
/// `haneul::scratch` is an ephemeral, per-transaction key-value store. Unlike `haneul::dynamic_field`,
...
/// way as a dynamic field name (see `haneul::dynamic_field::hash_type_and_key`).
///
...
fun hash_type_and_key<K: copy + drop>(k: K): address {
    haneul::dynamic_field::hash_type_and_key(DUMMY_ROOT, k)
}
