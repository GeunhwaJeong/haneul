...
pub struct HaneulInitArgs {
    ...
    /// Flag names match the field names of `FeatureFlags` in the haneul-protocol-config crate.
...
impl HaneulExtraValueArgs {
    ...
    fn parse_coin_reservation_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(HaneulExtraValueArgs::CoinReservation(amount, parsed_type))
    }
    ...
    fn parse_allowance_withdraw_value<'a, I: Iterator<Item = (ValueToken, &'a str)>>(
        ...
        Ok(HaneulExtraValueArgs::AllowanceWithdraw(
            amount,
            parsed_type,
            funder,
            fake_id,
...
impl HaneulValue {
    fn assert_move_value(self) -> MoveValue {
        match self {
            ...
            HaneulValue::CoinReservation(_, _) => {
                panic!("unexpected nested Haneul coin reservation in args")
            }
            HaneulValue::AllowanceWithdraw(_, _, _, _) => {
                panic!("unexpected nested Haneul allowance withdraw reservation in args")
            }
        }
    }
    ...
    pub(crate) fn into_call_arg(
        ...
        Ok(match self {
            ...
            HaneulValue::CoinReservation(amount, type_tag) => {
            ...
            HaneulValue::AllowanceWithdraw(amount, type_tag, funder, fake_id) => {
                let inner_type =
                    Balance::maybe_get_balance_type_param(&type_tag).ok_or_else(|| {
                        anyhow::anyhow!(
...
impl ParsableValue for HaneulExtraValueArgs {
    ...
    fn into_concrete_value(
        ...
        match self {
            ...
            HaneulExtraValueArgs::CoinReservation(amount, parsed_type) => {
                let type_tag = parsed_type.into_type_tag(mapping)?;
                Ok(HaneulValue::CoinReservation(amount, type_tag))
            }
            HaneulExtraValueArgs::AllowanceWithdraw(amount, parsed_type, funder, id) => {
                let type_tag = parsed_type.into_type_tag(mapping)?;
                let funder: HaneulAddress = funder.into_account_address(&|s| mapping(s))?.into();
                Ok(HaneulValue::AllowanceWithdraw(amount, type_tag, funder, id))
            }
        }
    }
}
