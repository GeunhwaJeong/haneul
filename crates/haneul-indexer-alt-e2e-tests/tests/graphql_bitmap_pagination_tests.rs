...
async fn alpha_cluster() -> (OffchainCluster, TempDir) {
    ...
    // `mock::haneul_system_state_inner_v2()` sets `protocol_version: 0`, which the
    // `kv_protocol_configs` pipeline rejects via `ProtocolConfig::get_for_version_if_supported`.
    const PROTOCOL_VERSION: u64 = 1;
    let system_state = HaneulSystemState::V2(HaneulSystemStateInnerV2 {
...
async fn graphql(cluster: &OffchainCluster, query: &str, variables: Value) -> Value {
    haneul_indexer_alt_e2e_tests::graphql::query(&cluster.graphql_url(), query, variables)
        .await
...
async fn paginate_forward(
    ...
    let query = r#"query($sender: HaneulAddress, $first: Int, $after: String) {
...
async fn paginate_backward(
    ...
    let query = r#"query($sender: HaneulAddress, $last: Int, $before: String) {
...
async fn transactions_page(
    ...
    let query = r#"query($sender: HaneulAddress, $first: Int, $last: Int, $after: String, $before: String) {
...
fn test_event(package: ObjectID, module: &str, sender: HaneulAddress, type_str: &str) -> Event {
    Event::new(
        &package,
        &Identifier::new(module).expect("valid module name"),
        sender,
        parse_haneul_struct_tag(type_str).expect("valid struct tag"),
        vec![],
    )
}
...
async fn test_sent_address() {
    ...
    let query = r#"query($address: HaneulAddress) {
...
async fn test_affected_object() {
    ...
    let query = r#"query($object: HaneulAddress) {
...
async fn test_events_type() {
    ...
    let by_instantiation = all_event_positions(
        &cluster,
        &json!({ "type": "0x42::m::T<0x2::haneul::HANEUL>" }),
    )
    .await;
...
async fn test_events_module_and_type_rejected() {
    ...
    let err = haneul_indexer_alt_e2e_tests::graphql::query(
        &cluster.graphql_url(),
        query,
