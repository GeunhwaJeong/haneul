...
```
cargo nextest run -p haneul-indexer-alt-e2e-tests --test transactional_tests
```
...
2. 1. Command to run all tests 
      ```
      test run -p haneul-indexer-alt-e2e-tests --test transactional_tests
      ```
   2. Command to run single test (replace `graphql/epochs/query.move` with the test you want to run)
      ```
      test run -p haneul-indexer-alt-e2e-tests --test transactional_tests graphql/epochs/query.move
      ```
