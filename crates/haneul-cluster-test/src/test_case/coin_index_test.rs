...
use haneul_types::coin::{COIN_MODULE_NAME, COIN_STRUCT_NAME};
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        let txn_data = {
            ...
            TestTransactionBuilder::new(account, gas_ref, gas_price)
                .transfer_haneul(Some(1_000_000), recipient)
                .build()
        };
        let response = ctx.sign_and_execute(txn_data, "transfer").await;
        let (owner_change, recipient_change) =
            Self::split_haneul_balance_changes(&response, account);

        let total_balance = Self::haneul_balance(ctx, account).await;
        let coin_object_count = Self::haneul_coins(ctx, account).await.len();
        ...
        // Classic `TransferHaneul` sends the recipient a distinct `Coin<HANEUL>`
        // object (address-balance accumulator crediting only applies to
        // `send_funds`-style transfers), so the legacy assertion holds: the
        // recipient owns exactly one HANEUL coin worth the transferred amount.
        ...
        let recipient_coins = Self::haneul_coins(ctx, recipient).await;
        assert_eq!(
            recipient_coins.len(),
            1,
            "classic TransferHaneul must yield exactly one recipient Coin<HANEUL>"
        );
        ...
        old_total_balance = Self::haneul_balance(ctx, account).await;
        ...
        let coin_object_count = Self::haneul_coins(ctx, account).await.len();
        assert_eq!(coin_object_count, old_coin_object_count);
        // The mint's gas cost reduces the HANEUL balance; just check it did not grow.
        ...
        // ListBalances (StateService::ListBalances) must report both HANEUL and the
        ...
        assert!(
            ...
            "list_balances should include HANEUL",
        );
        ...
        Self::call_managed(
            ...
            package.0,
            "take_from_envelope",
            vec![HaneulJsonValue::from_object_id(envelope.0)],
            gas_coin_id,
        )
        .await;
        assert_eq!(
        ...
        assert_eq!(
            ...
            "HANEUL balance should equal the sum of HANEUL coin values",
        );
        ...
        // once.
        let page_size = (haneul_coins.len() + 1) as u32;
...
impl CoinIndexTest {
    ...
    async fn coin_balance(ctx: &TestContext, owner: HaneulAddress, coin_type: &StructTag) -> u64 {
    ...
    async fn all_balances(ctx: &TestContext, owner: HaneulAddress) -> Vec<(String, u64)> {
    ...
    /// HANEUL (`Coin<0x2::haneul::HANEUL>`) coins owned by `owner`.
    ///
    /// NOTE: the owned-object filter is the *object* type `Coin<HANEUL>`
    /// (`GasCoin::type_()`), not the coin's inner type `0x2::haneul::HANEUL`
    /// (`GAS::type_()`, which `GetBalance`/`ListBalances` use as their `T`).
    async fn haneul_coins(ctx: &TestContext, owner: HaneulAddress) -> Vec<OwnedCoin> {
    ...
    async fn all_coins(ctx: &TestContext, owner: HaneulAddress) -> Vec<OwnedCoin> {
    ...
    async fn coins_of_type(
        ...
        loop {
            ...
            for object in &page.items {
                let balance = haneul_types::coin::Coin::extract_balance_if_coin(object)
                    .unwrap()
    ...
    /// Split the two HANEUL balance changes of a simple transfer into
    /// (owner_change, recipient_change) as (address, amount).
    fn split_haneul_balance_changes(
        response: &ExecutedTransaction,
        account: HaneulAddress,
    ) -> ((HaneulAddress, i128), (HaneulAddress, i128)) {
        let account_sdk: haneul_sdk_types::Address = account.into();
        let owner = response
            .balance_changes
            .iter()
        ...
        (
            (sdk_addr_to_haneul(&owner.address), owner.amount),
            (sdk_addr_to_haneul(&recipient.address), recipient.amount),
        )
    }

    fn type_matches(coin_type_str: &str, expected: &StructTag) -> bool {
        haneul_types::parse_haneul_struct_tag(coin_type_str)
            .map(|t| &t == expected)
            .unwrap_or(false)
    }
    ...
    async fn mint_managed(
        ...
        Self::call_managed(
            ...
            "mint",
            vec![
                HaneulJsonValue::from_object_id(cap),
                HaneulJsonValue::new(json!(amount.to_string())).unwrap(),
                HaneulJsonValue::new(json!(recipient)).unwrap(),
            ],
            gas_coin_id,
        )
        .await;
    ...
    /// refreshed each call). Reusing a stable gas coin keeps the account's HANEUL
...
fn sdk_addr_to_haneul(addr: &haneul_sdk_types::Address) -> HaneulAddress {
    (*addr).into()
}
...
fn managed_coin_type(pkg: ObjectID) -> StructTag {
    haneul_types::coin::Coin::type_(TypeTag::Struct(Box::new(managed_inner_type(pkg))))
}
