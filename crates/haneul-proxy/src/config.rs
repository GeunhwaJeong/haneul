...
/// metrics from. Peer membership is determined dynamically via Haneul gRPC calls to
/// a full node — haneul validator set and bridge committee are always resolved
...
pub struct DynamicPeerValidationConfig {
    /// url is the Haneul gRPC url we use to obtain valid peers on the blockchain.
    pub url: String,
