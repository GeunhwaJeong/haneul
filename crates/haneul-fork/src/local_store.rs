...
//! Fork-aware access to the embedded `haneul-rpc-store` data serves local data.
//!
...
use haneul_consistent_store::Restore as _;
...
use haneul_rpc_store::indexer::balance::Balance;
use haneul_rpc_store::indexer::object_by_owner::ObjectByOwner;
use haneul_rpc_store::indexer::object_by_type::ObjectByType;
use haneul_rpc_store::indexer::object_version_by_checkpoint::ObjectVersionByCheckpoint;
use haneul_rpc_store::indexer::objects::Objects;
use haneul_rpc_store::indexer::package_versions::PackageVersions;
use haneul_rpc_store::schema::checkpoint_contents;
use haneul_rpc_store::schema::checkpoint_seq_by_digest;
use haneul_rpc_store::schema::checkpoint_summary;
use haneul_rpc_store::schema::effects as schema_effects;
use haneul_rpc_store::schema::events as schema_events;
use haneul_rpc_store::schema::object_version_by_checkpoint;
use haneul_rpc_store::schema::objects;
use haneul_rpc_store::schema::objects::Status;
use haneul_rpc_store::schema::objects::TombstoneKind;
use haneul_rpc_store::schema::primitives::U64Be;
use haneul_rpc_store::schema::primitives::U64Varint;
use haneul_rpc_store::schema::transactions;
use haneul_rpc_store::schema::tx_metadata_by_seq;
use haneul_rpc_store::schema::tx_seq_by_digest;
...
const SEED_RESTORE_PIPELINE: &str = "haneul_fork_seed";

/// Fork-aware access to the local store backed up by `haneul-rpc-store`.
///
...
/// Local object removal that must be written as a `haneul-rpc-store` tombstone.
...
impl LocalStore {
    ...
    fn stage_checkpoint(
        &self,
        batch: &mut haneul_consistent_store::Batch,
        checkpoint: &VerifiedCheckpoint,
        contents: &CheckpointContents,
    ) -> anyhow::Result<()> {
        anyhow::ensure!(
    ...
    ///
    /// `haneul-rpc-store` indexes checkpoint summaries by checkpoint digest, so a content-digest
    ...
    /// Bulk-load the fork's seed objects and every index `haneul-rpc-store` derives from them, in one
    /// atomic batch.
    ///
    ...
    /// Stage every row `haneul-rpc-store` derives from one live object, covering the raw object
    ...
    /// Stage the object-version row using `haneul-rpc-store`'s restore helper.
    ...
    /// Stage the package-version row using `haneul-rpc-store`'s restore helper.
...
mod tests {
    ...
    fn make_object(id: ObjectID, version: u64, owner: Owner) -> Object {
        ...
        ObjectInner {
            ...
            previous_transaction: haneul_types::digests::TransactionDigest::genesis_marker(),
            storage_rebate: 0,
        }
        .into()
    }
    ...
    fn seed_load_writes_every_derived_index() {
        ...
        assert_eq!(
            store
                .schema
                .iter_objects_of_type(&haneul_rpc_store::schema::type_filter::TypeFilter::Type(
                    object_type,
                ))
                .unwrap()
                .count(),
