...
use haneul_types::storage::error::Kind as StorageErrorKind;
...
/// (`ReadStore`, `RpcStateReader`, `RpcIndexes`) so the same store backs the `haneul-rpc-api`
/// `RpcService`.
#[derive(Clone)]
...
impl ForkStore {
    /// Return the stock reader over the fork's local `haneul-rpc-store`, for reads the fork has no
    /// policy for.
    ///
...
// ============================================================================
//
// The fork serves RPC directly through `haneul-rpc-api` (it does not use
// `haneul-rpc-node`): [`crate::startup`] hands the store to `RpcService` as its
...
impl ReadStore for ForkStore {
    /// Read committee information from committed `haneul-rpc-store` data.
    ...
    /// Read unchanged runtime-loaded objects from committed `haneul-rpc-store` data alone, because the
    ...
    /// Read full checkpoint contents from committed `haneul-rpc-store` data alone, because the fork
...
impl RpcStateReader for ForkStore {
    ...
    /// Read a struct layout from `haneul-rpc-store`.
...
fn check_received_object(
    ...
    if object.owner != haneul_types::object::Owner::AddressOwner((*owner).into()) {
        return None;
    }
