...
pub async fn initialize(
    ...
    let system_state =
        haneul_types::haneul_system_state::get_haneul_system_state(&store).map_err(|e| {
            anyhow!(
...
/// Run the forked network. Spawns the `haneul-rpc-api` `RpcService` bound to `rpc_addr`, backed by the
...
pub async fn run(
    ...
    // Serve through `haneul-rpc-api`'s `RpcService` directly (the fork does not
    // depend on `haneul-rpc-node`). The `ForkStore` itself is the
