...
async fn test_current_object_read_saves_into_rpc_store_when_attached() {
    ...
    assert_eq!(
        haneul_types::storage::ObjectStore::get_object(&reader, &object_id),
        Some(object),
    );
    assert!(
        !temp
...
async fn test_index_reads_are_seed_bounded_and_never_scan_the_remote() {
    ...
    let seeded_owner = HaneulAddress::random_for_testing_only();
    let object_id = ObjectID::random();
    ...
    let unseeded = HaneulAddress::random_for_testing_only();
    assert_eq!(
...
fn test_rpc_get_coin_info_reads_seeded_type_index() {
    ...
    let metadata_object = Object::coin_metadata_for_testing(
        ...
        CoinMetadata {
            ...
            decimals: 9,
            name: "Haneul".to_owned(),
            symbol: "HANEUL".to_owned(),
            description: "Haneul gas coin".to_owned(),
            icon_url: None,
        },
    );
...
async fn test_rpc_latest_read_ignores_stale_cached_history() {
    ...
    let got = haneul_types::storage::ObjectStore::get_object(&reader, &object_id)
...
async fn test_advance_clock_executes_and_persists() {
    ...
    // Transaction rows are keyed by checkpoint position in `haneul-rpc-store`,
...
#[tokio::test]
async fn test_transfer_haneul_executes_and_persists() {
