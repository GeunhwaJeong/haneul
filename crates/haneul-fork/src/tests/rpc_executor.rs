...
impl TestHarness {
    ...
    /// Create transaction data where `amount` of HANEUL is transferred to `recipient`.
    fn build_transfer_tx_data_to(&self, recipient: HaneulAddress, amount: u64) -> TransactionData {
        let pt = {
    ...
    fn build_transfer_tx_data(&self, amount: u64) -> TransactionData {
        self.build_transfer_tx_data_to(HaneulAddress::random_for_testing_only(), amount)
    }
