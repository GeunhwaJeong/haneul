...
//! validates or creates fork metadata, opens the local `haneul-rpc-store` RocksDB instance, and
//! records the fork chain identifier before any readers are handed out.
//!
//! After the startup path creates Simulacrum, this module can start the embedded `haneul-rpc-store`
...
use haneul_rpc_store::CommitterLayer;
...
use haneul_rpc_store::Store;
use haneul_rpc_store::default_rocksdb_config;
use haneul_rpc_store::indexer::balance::Balance;
use haneul_rpc_store::indexer::epochs::Epochs;
use haneul_rpc_store::indexer::event_bitmap::EventBitmap;
...
use haneul_rpc_store::indexer::transaction_bitmap::TransactionBitmap;
...
const FORK_CHAIN_ID_PIPELINE: &str = "haneul_fork";
const METRICS_PREFIX: &str = "haneul_fork_rpc_store";
...
/// Opened fork services backed by `haneul-rpc-store`.
///
/// The manager owns the local RPC store and, once started, keeps the embedded `haneul-rpc-store`
...
impl ServiceManager {
    ...
    pub(crate) async fn start_indexer(
        ...
        let mut indexer = Indexer::from_store(
            ...
            ingestion_client,
            None,
            haneul_rpc_store::ConsistencyConfig::default(),
            None,
