...
//! GraphQL queries against the schema defined in `crates/haneul-indexer-alt-graphql/schema.graphql`,
//! built in modules for epochs, transactions, objects, and checkpoints.
//!
//! No GraphQL type escapes this module. Every query returns structures defined in this crate or
//! BCS-encoded data of runtime structures.
//!
...
pub(crate) mod available_range_query {
    ...
    /// Query the lowest available checkpoint sequence number for a given type (e.g., "Checkpoint"
    /// or "Transaction").
...
/// Coin types for which an address holds an accumulator balance at a checkpoint.
///
/// An address balance lives in a dynamic field under the accumulator root rather than on the
/// address, so it never appears in the address's owned-object enumeration. The field's object id is
/// derivable from `(address, coin type)`, but only once the coin type is known, and nothing local
/// knows which coin types an address has an accumulator balance in, including coin types it holds
/// no `Coin<T>` of. This query supplies exactly that list and nothing else. The balance amounts are
/// deliberately ignored, because the objects themselves are seeded and the stock `Balance` restore
/// pipeline derives the amounts from them.
pub(crate) mod address_balances_query {
    ...
    /// Coin types (the inner `T`, e.g. `0x2::haneul::HANEUL`) for which `address` has a non-zero
    /// accumulator balance at `checkpoint`.
    ///
    /// Rows with a zero or absent `addressBalance` are dropped, because the connection reports a
    /// row for every coin type the address has any balance in, including ones backed purely by coin
    /// objects, and those carry no accumulator field to seed.
    pub(crate) async fn query(
        address: HaneulAddressType,
        checkpoint: CheckpointSequenceNumber,
        client: &GraphQLClient,
    ...
    ///
    /// `BigInt` arrives as a decimal string. A malformed one is treated as "no balance" rather than
    /// an error, because the worst outcome is a field left unseeded, whereas failing here would
    /// take down seeding for the whole address over a single unparseable row.
...
pub(crate) mod object_seed_query {
    ...
    /// Resolve `(id, version, digest)` for objects at `checkpoint` without classifying their owner.
    ///
    /// The owner check in [`query`] is a policy on what a user may name as a seed. This entry point
    /// is for ids the fork derived itself and already knows the shape of. Today those are the
    /// accumulator balance fields, which are owned by the accumulator root rather than by any
    /// address, and so would be rejected by that check. Objects absent at the checkpoint come back
    /// as `None` rather than an error, because an address may simply have no field for a given coin
    /// type.
...
pub(crate) mod events_query {
    ...
    /// Fetch all events for a transaction by paginating through the GraphQL events connection.
    /// Returns `None` if the transaction doesn't exist. Returns an empty `TransactionEvents` if the
...
pub(crate) mod object_query {
    ...
    /// Fetch exact object versions, each returned only if it already existed at `checkpoint`.
    ///
    /// The checkpoint pins the *query scope* rather than each key, which is what lets these batch:
    /// `multiGetObjects` rejects a key carrying more than one bound, so a key with both a version
    /// and an `atCheckpoint` cannot be sent. Scoping instead keeps the guard intact — a scoped
    /// exact-version read resolves through `Object::at_version`, which drops any version created
    /// after the checkpoint in scope, so post-fork state still cannot leak into a diverged fork.
...
pub(crate) mod checkpoint_query {
    ...
    /// Fetch a checkpoint by sequence number (or the latest if `None`), returning both the verified
    /// summary envelope and the decoded checkpoint contents.
    pub(crate) async fn query(
