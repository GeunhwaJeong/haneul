...
//! live netork RPC and hands them to `haneul-rpc-store`'s `Restore` pipelines, which is where the
...
pub struct SeedInput {
    ...
    pub addresses: BTreeSet<HaneulAddress>,
...
pub(crate) struct SeedManifest {
    ...
    /// used.
    #[serde(default)]
    pub(crate) addresses: Vec<HaneulAddress>,
...
///
/// `coin_type` arrives as the inner type (`0x2::haneul::HANEUL`), while the accumulator keys its fields
/// on the wrapped `0x2::balance::Balance<T>`, so the id is derived from the wrapped form.
fn accumulator_field_id(address: HaneulAddress, coin_type: &str) -> Result<ObjectID, Error> {
...
async fn resolve_seeds(
    ...
    let addresses: Vec<HaneulAddress> = if input.addresses.is_empty() {
        Vec::new()
    } else {
...
mod tests {
    ...
    async fn prepare_seed_manifest_skips_address_seed_before_object_available_range() {
        ...
        let server = MockServer::start().await;
        let skipped_address = HaneulAddress::random_for_testing_only();
        let owner = HaneulAddress::random_for_testing_only();
        let object = Object::with_id_owner_version_for_testing(
            ObjectID::random(),
            SequenceNumber::from_u64(3),
    ...
    async fn prepare_seed_manifest_records_fully_scanned_addresses() {
        let server = MockServer::start().await;
        let owner = HaneulAddress::random_for_testing_only();
        let empty_owner = HaneulAddress::random_for_testing_only();
        let object = Object::with_id_owner_version_for_testing(
            ObjectID::random(),
            SequenceNumber::from_u64(3),
            Owner::AddressOwner(owner),
    ...
    async fn address_seed_pulls_in_the_accumulator_balance_field() {
        ...
        let field_id =
            accumulator_field_id(owner, coin_type).expect("field id should derive for HANEUL");
        let field = Object::with_id_owner_version_for_testing(
            field_id,
            SequenceNumber::from_u64(8),
            Owner::ObjectOwner(haneul_types::HANEUL_ACCUMULATOR_ROOT_OBJECT_ID.into()),
        );
    ...
    async fn address_seed_skips_coin_types_without_an_accumulator_balance() {
        ...
        Mock::given(method("POST"))
            ...
            .await;
        mock_address_objects(&server, 11, owner, &[]).await;
        mock_address_balances(&server, 11, owner, &[("0x2::haneul::HANEUL", 0)]).await;
    ...
    fn accumulator_field_id_keys_on_the_wrapped_balance_type() {
        ...
        assert_eq!(
            accumulator_field_id(owner, "0x2::haneul::HANEUL").unwrap(),
            *expected.inner(),
        );
        assert!(
