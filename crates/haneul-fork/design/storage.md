...
served from a stock `haneul-rpc-store` RocksDB, the same schema and indexes a real RPC node
...
{root}/
  ...
  rpc_store/                stock haneul-rpc-store RocksDB (RpcStoreSchema)
```
...
that we can simply embed the indexer from `haneul-rpc-store` and not worry about possible
future changes to the schemas in the `haneul-rpc-store` crate. It also allows to create custom
schemas if more data is required compared to the stock `haneul-rpc-store` columns. 
...
to the current schemas in `haneul-rpc-store`, it seems reasonable to use the same column families for both historical and locally executed data.
