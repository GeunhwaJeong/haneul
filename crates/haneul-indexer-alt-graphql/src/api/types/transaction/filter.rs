...
use haneul_rpc::proto::haneul::rpc::v2::TransactionFilter as GrpcTransactionFilter;
use haneul_rpc::proto::haneul::rpc::v2::TransactionLiteral;
use haneul_rpc::proto::haneul::rpc::v2::TransactionTerm;
use haneul_rpc::proto::haneul::rpc::v2::transaction_literal::Predicate;
...
impl TransactionFilter {
    ...
    pub(crate) fn to_grpc_filter(&self) -> Option<GrpcTransactionFilter> {
        ...
        if let Some(kind) = &self.kind {
            ...
            // (haneul-rpc-api/src/ledger_history/filter.rs).
            let zero_sender = sender_predicate(&HaneulAddress::ZERO);
...
fn sender_predicate(address: &HaneulAddress) -> Predicate {
    let mut f = SenderFilter::default();
    f.address = Some(address.to_string());
    Predicate::Sender(f)
}

fn affected_address_predicate(address: &HaneulAddress) -> Predicate {
    let mut f = AffectedAddressFilter::default();
    f.address = Some(address.to_string());
    Predicate::AffectedAddress(f)
}

fn affected_object_predicate(object: &HaneulAddress) -> Predicate {
...
mod tests {
    ...
    fn system_kind_maps_to_sender_zero_include() {
        ...
        assert_sender_literal(&literals[0], true, &HaneulAddress::ZERO.to_string());
    }
    ...
    fn programmable_kind_maps_to_sender_zero_negated() {
        ...
        let literals = term_literals(&proto);
        assert_eq!(literals.len(), 1);
        assert_sender_literal(&literals[0], false, &HaneulAddress::ZERO.to_string());
    }
    ...
    fn programmable_kind_combines_with_sent_address() {
        ...
        assert_sender_literal(&literals[1], false, &HaneulAddress::ZERO.to_string());
    }
}
