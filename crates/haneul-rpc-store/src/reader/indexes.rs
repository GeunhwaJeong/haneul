...
use haneul_types::storage::LedgerBitmapBucketIter;
...
enum BoundedBitmapIter<'d, K, V> {
    Fwd(haneul_consistent_store::Iter<'d, K, V>),
    Rev(haneul_consistent_store::RevIter<'d, K, V>),
}
...
mod tests {
    ...
    fn balance_iter_paginates_each_coin_type_once() {
        ...
        let mut cursor: Option<(HaneulAddress, StructTag)> = None;
