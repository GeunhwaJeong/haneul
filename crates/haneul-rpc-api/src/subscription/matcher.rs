...
//! with the same `haneul-inverted-index` visitors the persistent index uses; a
...
use haneul_inverted_index::BitmapKey;
...
/// have at least one filtered subscriber, using the same `haneul-inverted-index`
...
mod tests {
    ...
    fn addr(idx: u8) -> HaneulAddress {
        TestCheckpointBuilder::derive_address(idx)
    }

    fn sender_literal(address: HaneulAddress, negated: bool) -> proto::TransactionLiteral {
