...
use haneul_rpc_cursor::{CursorKind, CursorToken, Position};
...
pub fn boundary_watermark(position: Position, boundary: Option<u64>) -> Watermark {
    cursor_watermark(position, boundary, haneul_rpc_cursor::CursorKind::Boundary)
}
...
fn cursor_watermark(
    position: Position,
    boundary: Option<u64>,
    cursor_kind: haneul_rpc_cursor::CursorKind,
) -> Watermark {
    let cursor = match cursor_kind {
        haneul_rpc_cursor::CursorKind::Item => CursorToken::item(position),
        haneul_rpc_cursor::CursorKind::Boundary => CursorToken::boundary(position),
    };
    let mut wm = Watermark::default();
...
mod tests {
    use super::*;
    use haneul_rpc_cursor::{CursorKind, Position};
    ...
    fn options(ascending: bool) -> QueryOptions {
        ...
        request.ordering = Some(if ascending {
            haneul_rpc::proto::haneul::rpc::v2::Ordering::Ascending as i32
        } else {
            haneul_rpc::proto::haneul::rpc::v2::Ordering::Descending as i32
        });
