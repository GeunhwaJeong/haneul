...
impl TestCluster {
    ...
    pub fn observer_node(&self) -> Option<&haneul_swarm::memory::Node> {
    ...
    pub async fn wait_for_authenticator_state_update_for_providers(&self, jwk_ids: &[JwkId]) {
        timeout(Duration::from_secs(60), async {
            loop {
                let active: Vec<JwkId> = self.fullnode_handle.haneul_node.with(|node| {
    ...
    pub async fn wait_for_rpc_index_ready(&self) {
        let handle = &self.fullnode_handle.haneul_node;
