...
use haneul_move_build::{CompiledPackage, PackageDependencies};
...
use haneul_sdk::{
    HANEUL_DEVNET_URL, HANEUL_LOCAL_NETWORK_URL, HANEUL_LOCAL_NETWORK_URL_0, HANEUL_TESTNET_URL,
...
use haneul_source_verification::{ToolchainSource, VerifiedMetadata, verify_built, verify_source};
...
pub enum HaneulClientCommands {
    ...
    VerifySource {
        ...
        /// Rebuild with the `haneul` binary at this path instead of downloading a release. Skips
    ...
    pub async fn execute(
        ...
        let ret = match self {
            ...
            HaneulClientCommands::VerifyBytecodeMeter {
                ...
                let (pkg_name, modules) = match (module_paths, package_path) {
                    ...
                    (_, package_path) => {
                        ...
                        // Meter what `haneul client publish` would actually send, so build the
            ...
            HaneulClientCommands::SendFunds {
                ...
                let is_haneul = coin_type_tag == GAS::type_tag();
                ...
                let (amount, use_address_balance) = if let Some(amount) = amount {
                    let use_address_balance = if from_address_balance {
                            ...
                            "Insufficient address balance to send {amount} GEUNHWA. \
                        ...
                        bail!(
                            "Insufficient balance to send {amount} GEUNHWA. \
            ...
            HaneulClientCommands::Gas { address } => {
                ...
                // HANEUL the address holds directly. No gas object accounts for it, so it is
                // invisible in the coin list even though it is spendable. The balance API is
                // keyed by the coin type (`0x2::haneul::HANEUL`), not the object type `Coin<HANEUL>`.
                ...
                HaneulClientCommandResult::Gas(GasOutput::new(&coins, address_balance))
            }
            ...
            HaneulClientCommands::VerifySource {
                ...
                if let Some(on_chain_id) = verify_only {
                    ...
                    HaneulClientCommandResult::VerifySource(None)
                } else {
                    ...
                    let flavor = HaneulFlavor::with_client(context);
                    let publication =
                        read_publication::<HaneulFlavor>(&package_path, &environment, &flavor)
                            .await?
                            .with_context(|| {
                                format!(
                    ...
                    .await?;
                    HaneulClientCommandResult::VerifySource(Some(metadata))
                }
            }
...
impl Display for HaneulClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            HaneulClientCommandResult::Gas(gas) => {
                let gas_coins = &gas.gas_coins;
                if gas_coins.is_empty() && gas.address_geunhwa_balance == 0 {
                ...
                builder.push_record(vec![
                    ...
                    gas.address_geunhwa_balance.to_string(),
                    gas.address_haneul_balance.to_string(),
                ]);
                let mut table = builder.build();
            ...
            HaneulClientCommandResult::VerifySource(metadata) => {
...
impl Debug for HaneulClientCommandResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        let s = unwrap_err_to_string(|| match self {
            HaneulClientCommandResult::Gas(gas) => Ok(serde_json::to_string_pretty(gas)?),
            HaneulClientCommandResult::Object(object, json_content) => {
                let object = ObjectOutput::from_object_with_json(object, json_content.clone());
...
/// The gas coins owned by an address, together with the HANEUL the address holds directly rather
/// than in a coin object. The two are disjoint, so neither on its own is the address' full HANEUL.
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub struct GasOutput {
    pub gas_coins: Vec<GasCoinOutput>,
    pub address_geunhwa_balance: u64,
    pub address_haneul_balance: String,
}

impl GasOutput {
    pub fn new(gas_coins: &[GasCoin], address_geunhwa_balance: u64) -> Self {
        Self {
            gas_coins: gas_coins.iter().map(GasCoinOutput::from).collect(),
            address_geunhwa_balance,
            address_haneul_balance: format_balance(address_geunhwa_balance as u128, 9, 2, None),
        }
    }
}
...
fn order_balance_outputs_haneul_first(balances: &mut Vec<BalanceOutput>) {
    // The balance API reports the coin type (`0x2::haneul::HANEUL`), not the object type `Coin<HANEUL>`.
    let haneul_type_tag = GAS::type_().to_canonical_string(/* with_prefix */ true);
...
///
/// Every coin is merged into one, which is then sent by value. HANEUL is what pays for gas, so its
...
async fn send_all_coins(
    ...
    // For HANEUL the coins being sent are also what pays for gas, which a sponsor's gas coin cannot do:
    // it is the sponsor's amount that would end up with the recipient.
    let is_haneul = coin_type_tag == GAS::type_tag();
    ensure!(
        !is_haneul || gas_data.gas_sponsor.is_none(),
        "--all-coins cannot be used with a gas sponsor for HANEUL coin type, because the coins being \
    ...
    // Whatever is left in `coin_refs` is the gas payment, which is empty unless the coin is HANEUL.
    ...
    let target = if is_haneul {
        Argument::GasCoin
    } else {
        merged_args
            .pop()
            .context("non-HANEUL path always has at least one coin to merge into")?
    };
    ...
    // address balance, so no dust is left behind. Passing the HANEUL coins as an explicit gas payment
...
/// (empty payment, `ValidDuring` expiration), pays from their HANEUL coins otherwise, and when the
...
mod tests {
    ...
    fn gas_result(coins: usize, address_geunhwa_balance: u64) -> HaneulClientCommandResult {
        let gas_coins = (0..coins)
            .map(|i| GasCoinOutput {
                gas_coin_id: ObjectID::from_single_byte(i as u8),
                geunhwa_balance: 1_000_000_000,
                haneul_balance: format_balance(1_000_000_000, 9, 2, None),
            })
            .collect();
        HaneulClientCommandResult::Gas(GasOutput {
            gas_coins,
            address_geunhwa_balance,
            address_haneul_balance: format_balance(address_geunhwa_balance as u128, 9, 2, None),
        })
    ...
    fn gas_table_reports_address_balance_alongside_coins() {
        ...
        assert!(table.contains("haneulBalance (HANEUL)"), "{table}");
    ...
    fn balance_api_is_keyed_by_coin_type_not_object_type() {
        ...
        let haneul_balance = |coin_type: String| BalanceOutput {
            metadata: None,
        ...
        let mut balances = vec![
            haneul_balance("0x2::other::COIN".to_string()),
            haneul_balance(GAS::type_().to_canonical_string(true)),
        ];
        order_balance_outputs_haneul_first(&mut balances);
        assert_eq!(
            balances[0].balance.coin_type(),
            GAS::type_().to_canonical_string(true),
            "HANEUL should be ordered first"
        );
    }

    /// An address with no coins but a non-zero address balance still has spendable HANEUL.
    #[test]
