...
1. Procedural macros that replace `#[tokio::test]` and run test code inside a testing environment. These are `#[haneul_test]` and `#[sim_test]` and are documented below. The test harness created by these macros initializes the simulator runtime with a starting seed, generates the simulator configuration, and runs the test inside a newly created thread. Note that the simulator keeps its per-test state (fail-point registries, injection overrides, etc.) in process globals, so a process runs at most one simulation - to run many seeds, run one process per seed (see below).
...
- To run a test across many seeds, use `scripts/simtest/seed-search.py`, which runs each (test, seed) pair in its own process, e.g.:
        ...
        $ scripts/simtest/seed-search.py --package haneul-e2e-tests --num-seeds 10 my_test
...
Once you have found the bug or the source of the flakiness, you can validate your fix by running the test across many seeds:
      ...
      $ scripts/simtest/seed-search.py --package haneul-e2e-tests --num-seeds 20 my_flaky_test
