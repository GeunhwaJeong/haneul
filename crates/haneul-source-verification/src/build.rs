...
/// The JSON printed by `haneul move build --dump-bytecode-as-base64`. `digest` is intentionally
...
pub fn dump(
    ...
    if !help.contains("dump-bytecode-as-base64") {
        return Err(Error::BuildSubprocess {
            ...
            stderr: "this haneul version does not support --dump-bytecode-as-base64; \
    ...
    // locate the wallet through `HANEUL_CONFIG_DIR`. Without this the subprocess would consult the
    ...
    if let Some(dir) = client_config.and_then(|path| path.parent()) {
        command.env("HANEUL_CONFIG_DIR", dir);
    }
