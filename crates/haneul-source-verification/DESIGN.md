...
`haneul client verify-source <package-path> [--toolchain-version <version>]` decides whether a Move
...
verify_source(source_path, publication, toolchain_override, env, client, ...)
  ...
  ├─ ensure that toolchain's binary  (cached under the platform cache dir at
  │                                    `haneul/source-verification/binaries/<version>`, downloaded if absent)
...
- **The wrong linkage.** Module bytecode does not by itself pin the *versions* of the dependencies a
  ...
  linkage does not is running against different dependency code. Haneul's linkage is flat: when a package
...
- **The version matrix** (`tests/version_matrix.rs`, `#[ignore]`d) publishes a fixture with a
  ...
  `HANEUL_MATRIX_VERSIONS`; it is how the whole release history was swept, and is not run in CI.
