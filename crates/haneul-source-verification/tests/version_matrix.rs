...
//! Version matrix: check that a package published with a historical `haneul` binary verifies with the
//! current CLI.
//!
...
//! the current CLI first so `get_cargo_bin("haneul")` resolves:
//!
//! ```text
//! cargo build -p haneul
//! cargo test -p haneul-source-verification --test version_matrix -- --ignored --nocapture
//! ```
//!
//! [`era_matrix`] asserts on a small curated set. [`historical_sweep`] reports a table over whatever
//! versions are passed via `HANEUL_MATRIX_VERSIONS`; it does not assert.
...
use haneul_source_verification::{ToolchainSource, ensure_binary, verify_source};
...
/// revision they pin is not available from the haneul repository (`upload-pack: not our ref`).
/// `crates/haneul-framework-snapshot/manifest.json` records the revision, but the commit is not in the
...
#[tokio::test]
#[ignore = "downloads historical haneul release binaries and requires network; run manually"]
async fn era_matrix() {
    ...
    if let Ok(extra) = std::env::var("HANEUL_MATRIX_EXTRA_VERSIONS") {
        versions.extend(
            extra
                .split(',')
                .map(str::trim)
...
async fn historical_sweep() {
    ...
    let list = std::env::var("HANEUL_MATRIX_VERSIONS")
        .expect("set HANEUL_MATRIX_VERSIONS to a comma-separated list of versions to sweep");
...
async fn run_matrix(versions: &[String]) -> Vec<Outcome> {
    ...
    let config = cluster.swarm.dir().join(HANEUL_CLIENT_CONFIG);
    let current_haneul = get_cargo_bin("haneul");
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));

    let chain_id = chain_identifier(&current_haneul, &config);

    let mut outcomes = Vec::new();
    for version in versions {
        outcomes.push(run_one(version, &root, &config, &current_haneul, &chain_id).await);
    }
    outcomes
}
...
fn chain_identifier(haneul: &Path, config: &Path) -> String {
    let out = Command::new(haneul)
        .args(["client", "--client.config"])
        .arg(config)
        .args(["chain-identifier", "--format", "hex"])
        .output()
        .expect("could not run `haneul client chain-identifier`");

    assert!(
        out.status.success(),
        "`haneul client chain-identifier` failed: {}",
        diagnostics(&out)
    );
...
/// Publish the fixture with the `version` binary and verify it with `current_haneul`.
async fn run_one(
    version: &str,
    root: &Path,
    config: &Path,
    ...
    let old_haneul = match ensure_binary(version) {
        Ok(path) => path,
        Err(e) => {
    ...
    let modern = uses_current_package_system(&old_haneul);
    ...
    if let Err(note) = publish_fixture(&old_haneul, config, &sandbox, modern) {
        outcome.notes = note;
        return outcome;
    }
    ...
    match verify_one(modern, current_haneul, config, &sandbox, chain_id).await {
    ...
    outcome.tamper_detected = outcome.verified
        && verify_one(modern, current_haneul, config, &sandbox, chain_id)
            .await
            .is_err();
...
async fn verify_one(
    ...
    if modern {
        verify(current_haneul, config, sandbox)
    } else {
...
async fn verify_legacy(config: &Path, sandbox: &Path, chain_id: &str) -> Result<(), String> {
    ...
    let publication = read_publication::<HaneulFlavor>(sandbox, &localnet, &flavor)
        .await
...
fn verify(current_haneul: &Path, config: &Path, sandbox: &Path) -> Result<(), String> {
    let out = Command::new(current_haneul)
        .args(["client", "--client.config"])
        .arg(config)
        .args(["verify-source", "."])
        .current_dir(sandbox)
...
/// The haneul CLI prints some errors (notably dependency resolution) to stdout rather than stderr, so
