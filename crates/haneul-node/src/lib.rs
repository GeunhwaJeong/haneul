...
use haneul_config::transaction_deny_config::TransactionDenyRules;
...
use haneul_macros::{fail_point_arg, fail_point_async, replay_log};
...
pub struct HaneulNode {
    ...
    /// The embedded `haneul-rpc-store`, present when the node is a fullnode
    ...
    /// through [`HaneulNode::embedded_rpc_store`] for introspection.
...
impl HaneulNode {
    ...
    pub async fn start_async(
        ...
        // The embedded `haneul-rpc-store` is the node's index backend: when
        ...
        let mut embedded_rpc_store =
            if node_role.is_fullnode() && config.rpc().is_some_and(|rpc| rpc.enable_indexing()) {
                ...
                // The embedded `haneul-rpc-store` replaced the legacy `rpc-index`
    ...
    /// The embedded `haneul-rpc-store` index backend, when the node is a
...
///
/// The embedded `haneul-rpc-store` replaced the `RpcIndexStore` backend, which
...
async fn build_http_servers(
    ...
    // On top of haneul-http's hardened defaults (bounded concurrent streams;
    ...
    let server_config = {
        ...
        let mut server_config = haneul_http::Config::default()
...
///
/// `haneul_http::Builder::tls_single_cert` resolves the provider from rustls
...
/// `aws-config` in the `haneul` CLI), so the provider is pinned explicitly here
/// instead.
fn https_rustls_config(cert: &str, key: &str) -> Result<haneul_http::rustls::ServerConfig> {
    use haneul_http::rustls;
    use haneul_http::rustls::pki_types::pem::PemObject;
...
mod tests {
    ...
    async fn test_same_binary_version_does_not_recover() {
        ...
        HaneulNode::try_recover_forks(&checkpoint_store, &checkpoint_metrics, "v1").unwrap();
        assert!(
            checkpoint_store
                .get_checkpoint_fork_detected()
                .unwrap()
        ...
        HaneulNode::try_recover_forks(&checkpoint_store, &checkpoint_metrics, "v2").unwrap();
        assert!(
            checkpoint_store
                .get_checkpoint_fork_detected()
                .unwrap()
    ...
    async fn test_default_recovers() {
        ...
        let r =
            HaneulNode::check_and_recover_forks(&checkpoint_store, &checkpoint_metrics, None, "v2")
                .await;
        assert!(r.is_ok());
        assert!(
            checkpoint_store
    ...
    async fn test_checkpoint_overrides_clear_checkpoint_marker_only() {
        ...
        HaneulNode::try_recover_checkpoint_fork(&checkpoint_store, &ForkRecoveryConfig::default())
            .unwrap();
        assert!(
            checkpoint_store
                .get_checkpoint_fork_detected()
        ...
        HaneulNode::try_recover_checkpoint_fork(&checkpoint_store, &cfg).unwrap();
        assert!(
            checkpoint_store
                .get_checkpoint_fork_detected()
                .unwrap()
    ...
    async fn test_transaction_overrides_clear_transaction_marker() {
        ...
        HaneulNode::try_recover_transaction_fork(&checkpoint_store, &cfg).unwrap();
        assert!(
            checkpoint_store
                .get_transaction_fork_detected()
                .unwrap()
    ...
    async fn test_override_clears_fork_auto_recovery_refuses() {
        ...
        HaneulNode::check_and_recover_forks(
            &checkpoint_store,
            &checkpoint_metrics,
            Some(&cfg),
            "v1",
    ...
    async fn test_uncertified_transaction_fork_blocks_recovery() {
        ...
        HaneulNode::try_recover_forks(&checkpoint_store, &checkpoint_metrics, "v2").unwrap();
        HaneulNode::try_recover_forks(&checkpoint_store, &checkpoint_metrics, "v3").unwrap();
