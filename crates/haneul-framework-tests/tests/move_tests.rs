...
// `oracle-adapter/move` pins the Pyth Haneul contracts to commit `62c7a5b` (the
// resolved tip of the frozen `haneul-contract-testnet` branch), which it must, to
...
// the build. The package is verified locally instead: `haneul move build` is clean
// and `haneul move test` passes 13/13.
//
