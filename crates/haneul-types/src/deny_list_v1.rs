...
pub fn check_coin_deny_list_v1(
    ...
    funds_withdrawals: BTreeSet<(HaneulAddress, String)>,
...
pub(crate) fn addresses_by_coin_type_for_denylist_check(
    ...
    receiving_objects: &ReceivingObjects,
    funds_withdrawals: BTreeSet<(HaneulAddress, String)>,
) -> BTreeMap<String, BTreeSet<HaneulAddress>> {
    let mut addresses_by_coin_type: BTreeMap<String, BTreeSet<HaneulAddress>> = BTreeMap::new();
...
fn check_deny_list_v1_impl(
    ...
    addresses_by_coin_type: BTreeMap<String, BTreeSet<HaneulAddress>>,
    object_store: &dyn ObjectStore,
) -> UserInputResult {
    let addresses: BTreeSet<HaneulAddress> =
        addresses_by_coin_type.values().flatten().copied().collect();

    let denied_anywhere: BTreeSet<HaneulAddress> = addresses
        .into_iter()
