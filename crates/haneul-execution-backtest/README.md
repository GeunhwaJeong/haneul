...
It runs as a [`haneul-indexer-alt-framework`](../haneul-indexer-alt-framework) concurrent pipeline:
...
3. For each checkpoint, prefetches the package closure its transactions could load (batched
   ...
   a blocking worker — against reconstructed checkpoint state via the `haneul-execution` Executor.
...
- **System (framework) packages** (`0x1`, `0x2`, `0x3`, `0xb`, `0xdee9`): these keep a stable id but
  ...
  [`haneul-data-store`](../haneul-data-store) — the same `VersionQuery::AtCheckpoint` read `haneul-replay-2`
...
epoch RGP with the gasless compute cap, mirroring `haneul-transaction-checks`).
...
The crate is a thin orchestration layer: the [`haneul-indexer-alt-framework`](../haneul-indexer-alt-framework)
pipeline drives ingestion, scheduling, batching, and watermarks; this crate supplies a checkpoint
*processor* that reconstructs state and re-executes with the [`haneul-execution`](../../haneul-execution)
...
| `graphql.rs` | `GqlClient` — the one read gRPC can't serve: the epoch's framework packages *as of a checkpoint*, over `haneul-data-store`'s GraphQL `multiGetObjects`. |
...
flowchart TD
    ...
    Exec -->|EpochCtx.executor| VM[haneul-execution Executor]
