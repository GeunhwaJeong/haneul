...
/// budget/price *decision* (not the built `HaneulGasStatus`, which is large); the caller builds the
...
pub(crate) fn execute_one_transaction(
    ...
    let gas_status = match plan_gas(&gas_data, ctx) {
        ...
        GasPlan::Meter {
            ...
            match HaneulGasStatus::new(budget, price, ctx.reference_gas_price, &ctx.protocol_config)
            {
...
/// candidate coin types (every MoveCall type argument in the PTB, plus HANEUL) and matching.
///
...
fn rewrite_coin_reservations(
    ...
    // Candidate coin types `T` for `Balance<T>`: HANEUL, every MoveCall type argument, and the type
...
/// Decide how to meter, mirroring `haneul-transaction-checks::check_gas`: gasless transactions (empty
