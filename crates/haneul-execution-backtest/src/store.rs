...
use haneul_types::full_checkpoint_content::{Checkpoint, ObjectSet};
...
/// commands reference (via the system's own [`input_objects`](haneul_types::transaction::ProgrammableTransaction::input_objects)
...
impl RuntimeObjectResolver for ScanStore {
    fn read_child_object(
        ...
        // re-verify `parent` ownership (mirrors haneul-replay-2: these are reconstructions of
...
/// mirroring `haneul-replay-2`'s `get_input_objects_for_replay`. Returns an error if a declared input
