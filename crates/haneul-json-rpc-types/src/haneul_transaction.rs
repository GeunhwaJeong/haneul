                    ...
                    transactions: end_of_epoch_tx
                        ...
                        .map(|tx| match tx {
                            ...
                            EndOfEpochTransactionKind::ForwardingAddressRegistryCreate => {
                                HaneulEndOfEpochTransactionKind::ForwardingAddressRegistryCreate
                            }
                        })
                        .collect(),
                })
...
impl HaneulProgrammableTransactionBlock {
    ...
    async fn try_from_with_package_resolver(
        ...
        // failing the whole conversion. Matches the legacy `haneul-json-rpc` behavior and the
        // `haneul-indexer-alt-graphql` behavior at `programmable/mod.rs`.
...
mod tests {
    ...
    async fn programmable_transaction_falls_back_when_layout_resolution_fails() {
        ...
        let recipient = builder.pure(HaneulAddress::ZERO).unwrap();
        builder.programmable_move_call(
            ObjectID::ZERO,
            ident_str!("pay").to_owned(),
            ident_str!("pay_all_haneul").to_owned(),
            vec![],
            vec![recipient],
        );

        let resolver = Resolver::new(EmptyPackageStore);
        let transaction = HaneulProgrammableTransactionBlock::try_from_with_package_resolver(
            builder.finish(),
            &resolver,
        )
        .await
        ...
        let HaneulCallArg::Pure(input) = &transaction.inputs[0] else {
            panic!("expected pure input");
        };
        assert_eq!(input.value_type(), None);

        // HaneulAddress::ZERO BCS-encodes to 32 zero bytes. With no layout, those bytes should come
