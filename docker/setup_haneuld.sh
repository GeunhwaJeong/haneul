...
GENESIS_FILE="$HOME"/.haneul/config/genesis.json
if [ -f "$GENESIS_FILE" ]; then
  echo "$GENESIS_FILE exists..."
else
  echo "$GENESIS_FILE does not exist. Generating..."

  haneuld init --chain-id "$CHAIN_ID" "$MONIKER"
  # staking/governance token is hardcoded in config, change this
  sed -i "s/\"uhaneul\"/\"$STAKE\"/" "$GENESIS_FILE"
  ...
  sed -i 's/keyring-backend = "os"/keyring-backend = "test"/' "$HOME"/.haneul/config/client.toml
fi

APP_TOML_CONFIG="$HOME"/.haneul/config/app.toml
APP_TOML_CONFIG_NEW="$HOME"/.haneul/config/app_new.toml
CONFIG_TOML_CONFIG="$HOME"/.haneul/config/config.toml
...
if ! haneuld keys show validator $KEYRING; then
  (echo "$PASSWORD"; echo "$PASSWORD") | haneuld keys add validator $KEYRING

  # hardcode the validator account for this instance
  echo "$PASSWORD" | haneuld genesis add-genesis-account validator "1000000000$STAKE,1000000000$FEE" $KEYRING

  # (optionally) add a few more genesis accounts
  for addr in "$@"; do
    echo $addr
    haneuld genesis add-genesis-account "$addr" "1000000000$STAKE,1000000000$FEE,5000000000uusd"
  done

  # submit a genesis validator tx
  ## Workraround for https://github.com/cosmos/cosmos-sdk/issues/8251
  (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | haneuld genesis gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID" --amount="250000000$STAKE" $KEYRING
  ## should be:
  # (echo "$PASSWORD"; echo "$PASSWORD"; echo "$PASSWORD") | haneuld genesis gentx validator "250000000$STAKE" --chain-id="$CHAIN_ID"
