...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=haneul \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=haneuld \
...
install: go-cache
	@echo "🔄 - Installing Haneul..."
	@go install $(BUILD_FLAGS) -mod=readonly ./cmd/haneuld
	@echo "✅ - Installed Haneul successfully! Run it using 'haneuld'!"
	@echo ""
...
build: go-cache
	...
	@if [ "$(OS)" = "Windows_NT" ]; then \
		GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o bin/haneuld.exe ./cmd/haneuld; \
	else \
		go build -mod=readonly $(BUILD_FLAGS) -o bin/haneuld ./cmd/haneuld; \
	fi
	@echo "✅ - Built Haneul successfully! Run it using './bin/haneuld'!"
	@echo ""
	@echo "====== Install Summary ======"
	@echo "Haneul: $(VERSION)"
	@echo "Cosmos SDK: $(COSMOS_SDK_VERSION)"
...
test-node:
	CHAIN_ID="local-1" HOME_DIR="~/.haneul" TIMEOUT_COMMIT="500ms" CLEAN=true sh scripts/test_node.sh
...
ictest-basic: rm-testcache
	cd interchaintest && go test -race -v -run TestBasicHaneulStart .

ictest-statesync: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulStateSync .

ictest-ibchooks: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulIBCHooks .

ictest-tokenfactory: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulTokenFactory .

ictest-feeshare: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulFeeShare .
...
ictest-globalfee: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulGlobalFee .

ictest-upgrade: rm-testcache
	cd interchaintest && go test -race -v -run TestBasicHaneulUpgrade .

ictest-ibc: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulGaiaIBCTransfer .

ictest-unity-deploy: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulUnityContractDeploy .

ictest-drip: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulDrip .

ictest-feepay: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulFeePay .

ictest-burn: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulBurnModule .

ictest-cwhooks: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulCwHooks .

ictest-clock: rm-testcache
	cd interchaintest && go test -race -v -run TestHaneulClock .
...
local-image: install-heighliner
	...
	heighliner build --chain haneul --local -f ./chains.yaml
