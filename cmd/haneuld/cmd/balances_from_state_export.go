...
// https://github.com/GeunhwaJeong/haneul/blob/main/cmd/osmosisd/cmd/balances_from_state_export.go
...
Example:
	haneuld export-derive-balances ../genesis.json ../snapshot.json
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Produce the map of address to total haneul balance, both staked and UnbondingStake
			...
			for _, unbonding := range stakingGenesis.UnbondingDelegations {
				...
				unbondingHaneuls := sdkmath.NewInt(0)
				for _, entry := range unbonding.Entries {
					unbondingHaneuls = unbondingHaneuls.Add(entry.Balance)
				}

				acc.UnbondingStake = acc.UnbondingStake.Add(unbondingHaneuls)
			...
			for _, delegation := range stakingGenesis.Delegations {
				...
				stakedHaneul := delegation.Shares.MulInt(val.Tokens).Quo(val.DelegatorShares).RoundInt()

				acc.Staked = acc.Staked.Add(stakedHaneul)
			...
			for addr, account := range snapshotAccs {
				...
				account.TotalBalances = sdk.NewCoins().
					...
					Add(sdk.NewCoin("uhaneul", account.Staked)).
					Add(sdk.NewCoin("uhaneul", account.UnbondingStake)).
					Add(account.Bonded...)
...
Example:
	haneuld staked-to-csv ../balances.json ../airdrop.csv
`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx := client.GetClientContextFromCmd(cmd)
